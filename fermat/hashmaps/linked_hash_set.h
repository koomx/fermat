...
namespace fermat {
    ...
    template <
        typename Key, typename KeyHash = typename fermat::flat_hash_set<Key>::hasher,
        typename KeyEq = typename fermat::flat_hash_set<Key, KeyHash>::key_equal,
        typename Alloc = std::allocator<Key>>
    class linked_hash_set {
        using KeyArgImpl = fermat::base_internal::KeyArg<
            fermat::base_internal::IsTransparent<KeyEq>::value && fermat::base_internal::IsTransparent<KeyHash>::value>;
    ...
    private:
        ...
        using SetType = fermat::flat_hash_set<typename ListType::iterator, Wrapped<hasher>,
        ...
        private:
            ...
            explicit NodeHandle(ListType list)
                : list_(std::move(list)) { }
            ListType list_;
        };
    ...
    public:
        ...
        template <typename K = key_type,
            std::enable_if_t<
                !std::is_convertible_v<const key_arg<K>&, const_iterator> && !std::is_convertible_v<const key_arg<K>&, iterator>,
                int> = 0>
        ...
        template <
            ...
            std::enable_if_t<!std::is_convertible_v<key_arg<K>&&, const_iterator> && !std::is_convertible_v<key_arg<K>&&, iterator>,
                int> = 0>
    ...
    private:
        ...
        std::pair<iterator, bool> EmplaceInternal(const_iterator hint,
            ...
            auto list_iter = node_donor.emplace(node_donor.end(), std::forward<Args>(args)...);
