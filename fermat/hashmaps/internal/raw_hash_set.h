...
#include <fermat/base/common.h> // IWYU pragma: export // for node_handle
#include <fermat/base/common_policy_traits.h>
#include <fermat/hashmaps/internal/hash_function_defaults.h>
#include <fermat/hashmaps/internal/hash_policy_traits.h>
#include <fermat/hashmaps/internal/hashtable_control_bytes.h>
#include <fermat/hashmaps/internal/hashtable_debug_hooks.h>
...
#elif (KUMO_HAVE_ADDRESS_SANITIZER || KUMO_HAVE_HWADDRESS_SANITIZER || KUMO_HAVE_MEMORY_SANITIZER) && !defined(NDEBUG_SANITIZER) // If defined, performance is important.
...
#define FERMAT_SWISSTABLE_ENABLE_GENERATIONS
#endif

#ifdef FERMAT_SWISSTABLE_ASSERT
#error FERMAT_SWISSTABLE_ASSERT cannot be directly set
#else
// We use this macro for assertions that users may see when the table is in an
// invalid state that sanitizers may help diagnose.
#define FERMAT_SWISSTABLE_ASSERT(CONDITION) \
    assert((CONDITION) && "Try enabling sanitizers.")
        ...
        inline void* DefaultIterSlot() {
            return &kDefaultIterSlot;
        }
        ...
        constexpr bool IsValidCapacity(size_t n) {
            return ((n + 1) & n) == 0 && n > 0;
        }
        ...
        constexpr size_t CapacityToGrowth(size_t capacity) {
            FERMAT_SWISSTABLE_ASSERT(IsValidCapacity(capacity));
            // `capacity*7/8`
        ...
        class HashtableCapacityImpl {
            using IntType = std::conditional_t<StorageMode == kCapacityByValue, size_t, uint8_t>;
        ...
        public:
            ...
            explicit constexpr HashtableCapacityImpl(size_t capacity)
                ...
                FERMAT_SWISSTABLE_ASSERT(capacity == 0 || IsValidCapacity(capacity));
            }
            ...
            constexpr size_t capacity() const {
                FERMAT_SWISSTABLE_ASSERT(IsValid());
        ...
        private:
            ...
            explicit constexpr HashtableCapacityImpl(InvalidCapacity capacity)
                ...
                FERMAT_SWISSTABLE_ASSERT(capacity_data_ > kAboveMaxValidCapacity);
            }
        ...
        public:
            ...
            static constexpr size_t kMaxBlockedElementCount = (uint64_t { 1 } << kBlockedElementBitCount) - 1;
            static constexpr size_t kSizeBitCount = 64 - (kBlockedElementBitCount + PerTableSeed::kBitCount + (StorageMode == kCapacityByValue ? 0 : sizeof(HashtableCapacity) * 8));
            ...
            void set_size(size_t size) {
                data_ = (data_ & kMetadataMask) | (static_cast<uint64_t>(size) << kSizeShift);
            }
            ...
            void init_blocked_element_count(uint64_t count) {
                FERMAT_SWISSTABLE_ASSERT(blocked_element_count() == 0);
                FERMAT_SWISSTABLE_ASSERT(count <= kMaxBlockedElementCount);
        ...
        private:
            ...
            static constexpr size_t kDataBitCount = PerTableSeed::kBitCount + kSizeBitCount + kBlockedElementBitCount;
            ...
            static constexpr uint64_t kSeedMask = (uint64_t { 1 } << PerTableSeed::kBitCount) - 1;
        ...
        static_assert(
            sizeof(HashtableInlineDataImpl<kCapacityByValue>::HashtableCapacity) == sizeof(size_t));
        ...
        static_assert(kMaxBlockedElementsForLargeTables <= HashtableInlineData::kMaxBlockedElementCount);
        ...
        public:
            ...
            explicit GrowthInfoAccessor(void* control)
                : growth_info_lower_bound_(reinterpret_cast<uint8_t*>(control) - 1 - NumGenerationBytes()) { }
            ...
            void OverwriteEmptyAsFull() {
                FERMAT_SWISSTABLE_ASSERT(GetGrowthLeftLowerBound() > 0);
            ...
            void OverwriteControlAsFull(ctrl_t ctrl) {
                FERMAT_SWISSTABLE_ASSERT(GetGrowthLeftLowerBound() >= static_cast<size_t>(IsEmpty(ctrl)));
        ...
        public:
            explicit RawHashSetLayout(size_t capacity, size_t slot_size,
                ...
                FERMAT_SWISSTABLE_ASSERT(
                    slot_size <= ((std::numeric_limits<size_t>::max)() - slot_offset_) / capacity);
                control_offset_ += slot_array_padding;
                generation_offset_ += slot_array_padding;
                FERMAT_SWISSTABLE_ASSERT(!IsSmallCapacity(capacity) || control_offset_ == slot_offset_);
        ...
        constexpr size_t MaxSooSlotSize() {
            return sizeof(HeapPtrs);
        }
        ...
        public:
            ...
            ctrl_t* control() const {
                FERMAT_SWISSTABLE_ASSERT(capacity() > 0);
            ...
            void* slot_array(size_t capacity) const {
                FERMAT_SWISSTABLE_ASSERT(capacity == this->capacity());
                ctrl_t* ctrl = control();
            ...
            void increment_size() {
                FERMAT_SWISSTABLE_ASSERT(size() < capacity());
                inline_data_.increment_size();
            }
            void increment_size(size_t n) {
                FERMAT_SWISSTABLE_ASSERT(size() + n <= capacity());
                inline_data_.increment_size(n);
            }
            void decrement_size() {
                FERMAT_SWISSTABLE_ASSERT(!empty());
            ...
            HashtableCapacity capacity_impl() const {
                ...
                FERMAT_SWISSTABLE_ASSERT(cap.IsValid());
                return cap;
            }
            ...
            GrowthInfoAccessor growth_info() const {
                FERMAT_SWISSTABLE_ASSERT(GrowthInfoSizeForCapacity(capacity()) > 0);
        ...
        private:
            ...
            void AssertInSooMode() const {
                FERMAT_SWISSTABLE_ASSERT(capacity() == SooCapacity());
            }
        ...
        size_t SelectReservationSizeForIterRange(InputIter first, InputIter last,
            ...
            if (turbo::meta_internal::IsAtLeastIterator<std::random_access_iterator_tag,
                    InputIter>()) {
...
#if defined(FERMAT_SWISSTABLE_ENABLE_GENERATIONS) || KUMO_OPTION_HARDENED == 1 || !defined(NDEBUG)
            return true;
#else
            return false;
#endif
        ...
        inline void AssertIsFull(const ctrl_t* const& ctrl, const void* slot,
            ...
            if (SwisstableGenerationsEnabled()) {
                if (KUMO_UNLIKELY(generation != CrashIfIteratorIsInvalid(generation_ptr))) {
                    TURBO_RAW_LOG(FATAL,
        ...
        inline void AssertIsValidForComparison(const ctrl_t* const& ctrl,
            ...
            if (!SwisstableDebugEnabled())
                return;
            const bool ctrl_is_valid_for_comparison = slot == nullptr || slot == DefaultIterSlot() || IsFull(CrashIfIteratorIsInvalid(ctrl));
            if (SwisstableGenerationsEnabled()) {
                if (KUMO_UNLIKELY(generation != CrashIfIteratorIsInvalid(generation_ptr))) {
                ...
                KUMO_HARDENING_ASSERT_SLOW(
                    ctrl_is_valid_for_comparison && "Invalid iterator comparison. The element might have been erased or "
        ...
        inline void AssertSameContainer(const ctrl_t* const& ctrl_a,
            ...
            if (SwisstableGenerationsEnabled()) {
                if (KUMO_LIKELY(generation_ptr_a == generation_ptr_b))
                    return;
                ...
                KUMO_HARDENING_ASSERT_SLOW(
                    AreItersFromSameContainer(ctrl_a, ctrl_b, slot_a, slot_b) && "Invalid iterator comparison. The iterators may be from different "
        ...
        inline size_t TryFindNewIndexWithoutProbing(size_t h1, size_t old_index,
            ...
            FERMAT_SWISSTABLE_ASSERT(((old_index - h1) & old_capacity) >= Group::kWidth);
        ...
        using DeallocBackingArrayFn = decltype(&DeallocateBackingArray<8, std::allocator<char>>);
        ...
        constexpr size_t MaxValidSizeForKeySize(size_t key_size) {
            if (key_size < kSizeOfSizeT)
                return size_t { 1 } << 8 * key_size;
        ...
        constexpr size_t MaxSmallAfterSooCapacity() {
            return 7;
        }
        ...
        struct InstantiateRawHashSet {
            using type = typename fermat::base_internal::ApplyWithoutDefaultSuffix<
                raw_hash_set,
                fermat::base_internal::TypeList<void, typename Policy::DefaultHash, typename Policy::DefaultEq,
                    typename Policy::DefaultAlloc>,
                fermat::base_internal::TypeList<Policy, Hash, Eq, Alloc>>::type;
        };
        ...
        class raw_hash_set {
            ...
            using Hash = base_internal::GetFromListOr<typename Policy::DefaultHash, 0, Params...>;
            using Eq = base_internal::GetFromListOr<typename Policy::DefaultEq, 1, Params...>;
            using Alloc = base_internal::GetFromListOr<typename Policy::DefaultAlloc, 2, Params...>;
            using KeyArgImpl = base_internal::KeyArg<base_internal::IsTransparent<Eq>::value && base_internal::IsTransparent<Hash>::value>;
        ...
        private:
            ...
            constexpr static bool kIsDefaultHash = std::is_same_v<hasher, turbo::Hash<key_type>> || std::is_same_v<hasher, fermat::container_internal::StringHash>;
            ...
            using key_hash_result = turbo::remove_cvref_t<decltype(std::declval<const Hash&>()(
            ...
            template <class T>
            using IsDecomposableAndInsertable = IsDecomposable<std::enable_if_t<Insertable<T>::value, T>>;
            ...
            using IsLifetimeBoundAssignmentFrom = std::conditional_t<
                fermat::base_internal::policy_trait_element_is_owner<Policy>::value, std::false_type,
                turbo::type_traits_internal::IsLifetimeBoundAssignment<init_type, U>>;
        ...
        public:
            ...
            public:
                ...
                iterator()
                    : slot_(static_cast<slot_type*>(DefaultIterSlot())) { }
                ...
                iterator& operator++() {
                    ...
                    if (KUMO_UNLIKELY(*ctrl_ == ctrl_t::kSentinel))
                        slot_ = nullptr;
                    return *this;
                }
                // PRECONDITION: not an end() iterator.
            ...
            class const_iterator {
                ...
                friend struct fermat::container_internal::hashtable_debug_internal::
            ...
            using node_type = fermat::base_internal::node_handle<Policy, hash_policy_traits<Policy>, Alloc>;
            using insert_return_type = fermat::base_internal::InsertReturnType<iterator, node_type>;

            // Note: can't use `= default` due to non-default noexcept (causes
            // problems for some compilers). NOLINTNEXTLINE
            raw_hash_set() noexcept(
                std::is_nothrow_default_constructible_v<hasher> && std::is_nothrow_default_constructible_v<key_equal> && std::is_nothrow_default_constructible_v<allocator_type>) { }
            ...
            //   // copies the strings into the set.
            //   fermat::flat_hash_set<std::string> s = {"abc", "def"};
            //
            ...
            //
            //   fermat::flat_hash_set<int> a, b{a};
            //
            ...
            raw_hash_set(const raw_hash_set& that)
                : raw_hash_set(that, AllocTraits::select_on_container_copy_construction(allocator_type(that.char_alloc_ref()))) { }
            ...
            KUMO_ATTRIBUTE_NOINLINE raw_hash_set(raw_hash_set&& that) noexcept(
                std::is_nothrow_copy_constructible_v<hasher> && std::is_nothrow_copy_constructible_v<key_equal> && std::is_nothrow_copy_constructible_v<allocator_type>)
            ...
            raw_hash_set& operator=(raw_hash_set&& that) noexcept(
                AllocTraits::is_always_equal::value && std::is_nothrow_move_assignable_v<hasher> && std::is_nothrow_move_assignable_v<key_equal>) {
            ...
            iterator begin() KUMO_ATTRIBUTE_LIFETIME_BOUND {
                if (KUMO_UNLIKELY(empty()))
                    return end();
                if (is_small())
                    return single_iterator();
                iterator it = { control(), slot_array(capacity()),
                    common().generation_ptr() };
                it.skip_empty_or_deleted();
                FERMAT_SWISSTABLE_ASSERT(IsFull(*it.control()));
                return it;
            }
            ...
            template <class T,
                int = std::enable_if_t<IsDecomposableAndInsertable<T>::value && IsNotBitField<T>::value && !IsLifetimeBoundAssignmentFrom<T>::value,
                    int>()>
            ...
            template <class T, int&...,
                std::enable_if_t<IsDecomposableAndInsertable<T>::value && IsNotBitField<T>::value && IsLifetimeBoundAssignmentFrom<T>::value,
                    int> = 0>
            ...
            //   s.insert(p);
            //
            template <class T, int = std::enable_if_t<IsDecomposableAndInsertable<const T&>::value && !IsLifetimeBoundAssignmentFrom<const T&>::value, int>()>
            std::pair<iterator, bool> insert(const T& value)
                KUMO_ATTRIBUTE_LIFETIME_BOUND {
                return emplace(value);
            }
            template <class T, int&...,
                std::enable_if_t<IsDecomposableAndInsertable<const T&>::value && IsLifetimeBoundAssignmentFrom<const T&>::value,
                    int> = 0>
            std::pair<iterator, bool> insert(
            ...
            insert_return_type insert(node_type&& node) KUMO_ATTRIBUTE_LIFETIME_BOUND {
                ...
                const auto& elem = PolicyTraits::element(fermat::base_internal::CommonAccess::GetSlot(node));
                auto res = PolicyTraits::apply(
                    InsertSlot<false> { *this, std::move(*fermat::base_internal::CommonAccess::GetSlot(node)) },
                    elem);
                if (res.second) {
                    fermat::base_internal::CommonAccess::Reset(&node);
            ...
            public:
                ...
                void operator()(Args&&... args) const {
                    FERMAT_SWISSTABLE_ASSERT(*slot_);
            ...
            node_type extract(const_iterator position) {
                ...
                allocator_type alloc(char_alloc_ref());
                auto node = fermat::base_internal::CommonAccess::Transfer<node_type>(alloc, position.slot());
            ...
            void swap(raw_hash_set& that) noexcept(
                AllocTraits::is_always_equal::value && std::is_nothrow_swappable_v<hasher> && std::is_nothrow_swappable_v<key_equal>) {
                AssertNotDebugCapacity();
                that.AssertNotDebugCapacity();
                using std::swap;
                swap_common(that);
            ...
            friend bool operator==(const raw_hash_set& a, const raw_hash_set& b) {
                ...
                for (const value_type& elem : *outer) {
                    ...
                    static constexpr bool kKeyEqIsValueEq = std::is_same_v<key_type, value_type> && std::is_same_v<key_equal, hash_default_eq<key_type>>;
            ...
            TurboHashValue(H h, const raw_hash_set& s) {
                return H::combine(H::combine_unordered(std::move(h), s.begin(), s.end()),
                    turbo::hash_internal::WeaklyMixedInteger { s.size() });
            }
        ...
        private:
            ...
            friend struct fermat::container_internal::HashtableFreeFunctionsAccess;
            ...
            iterator find_large(const key_arg<K>& key) {
                ...
                while (true) {
                    ...
                    if (KUMO_LIKELY(g.MaskEmpty()))
                        return end();
                    seq.next();
                    FERMAT_SWISSTABLE_ASSERT(seq.index() <= cap && "full table!");
                }
            }

            void clear_backing_array(bool reuse) {
                FERMAT_SWISSTABLE_ASSERT(capacity() > kMaxSmallCapacity);
            ...
            void dealloc() {
                FERMAT_SWISSTABLE_ASSERT(capacity() > DefaultCapacity());
                DeallocBackingArray(common(), sizeof(slot_type), alignof(slot_type),
                    get_dealloc_backing_array_fn(), &char_alloc_ref());
            }

            void destructor_impl() {
                if (SwisstableGenerationsEnabled() && maybe_invalid_capacity().IsMovedFrom()) {
                    return;
                }
                if constexpr (SooEnabled()) {
                    if (is_small() && (PolicyTraits::template destroy_is_trivial<Alloc>() || empty())) {
                        return;
                    }
            ...
            void annotate_for_bug_detection_on_move([[maybe_unused]] raw_hash_set& that) {
                ...
                if (!SwisstableGenerationsEnabled() || !maybe_invalid_capacity().IsValid() || capacity() == DefaultCapacity()) {
                    return;
                }
            ...
            find_or_prepare_insert_soo(const K& key) {
                ...
                FERMAT_SWISSTABLE_ASSERT(capacity() == 1);
                constexpr bool kUseMemcpy = PolicyTraits::transfer_uses_memcpy() && SooEnabled();
            ...
            std::pair<slot_type*, bool> find_or_prepare_insert_large(const K& key) {
                ...
                while (true) {
                    ...
                    seq.next();
                    FERMAT_SWISSTABLE_ASSERT(seq.index() <= capacity() && "full table!");
                }
            }
        ...
        protected:
            ...
            void AssertHashEqConsistent(const K& key) {
                ...
                if (std::is_same_v<hasher, fermat::container_internal::StringHash> && std::is_same_v<key_equal, fermat::container_internal::StringEq>) {
                    return;
                }
                if (std::is_scalar_v<key_type> && std::is_same_v<hasher, turbo::Hash<key_type>> && std::is_same_v<key_equal, std::equal_to<key_type>>) {
                    return;
                }
                if (empty())
                    return;
                ...
                const auto assert_consistent = [&](const ctrl_t*, void* slot) {
                    ...
                    [[maybe_unused]] const bool is_hash_equal = hash_of_arg == hash_of(to_slot(slot));
            ...
            void emplace_at(slot_type* slot, Args&&... args) {
                ...
                assert((is_small() || PolicyTraits::apply(FindElement { *this }, PolicyTraits::element(slot)).slot() == slot) && "constructed value does not match the lookup key");
            }
        ...
        private:
            ...
            static void transfer_unprobed_elements_to_next_capacity_fn(
                ...
                for (size_t group_index = 0; group_index < old_capacity;
                    ...
                    for (auto in_fixed_group_index : old_g.MaskFull()) {
                        ...
                        FERMAT_SWISSTABLE_ASSERT((new_index & old_capacity) <= old_index);
            ...
            static const PolicyFunctions& GetPolicyFunctions() {
                ...
                static_assert(alignof(slot_type) <= size_t { (std::numeric_limits<uint16_t>::max)() });
                static_assert(sizeof(key_type) <= size_t { (std::numeric_limits<uint32_t>::max)() });
                static_assert(sizeof(value_type) <= size_t { (std::numeric_limits<uint32_t>::max)() });
                static constexpr size_t kBackingArrayAlignment = BackingArrayAlignment(alignof(slot_type));
        ...
        struct HashtableFreeFunctionsAccess {
            ...
            static typename Set::size_type EraseIf(Predicate& pred, Set* c) {
                ...
                if (c->is_small()) {
                    ...
                    if (!pred(*it)) {
                        FERMAT_SWISSTABLE_ASSERT(c->size() == 1 && "hash table was modified unexpectedly");
                        return 0;
                    }
                    c->destroy(it.slot());
                ...
                FERMAT_SWISSTABLE_ASSERT(original_size_for_assert - num_deleted == c->size() && "hash table was modified unexpectedly");
                return num_deleted;
            }
            ...
            static void ForEach(Callback& cb, Set* c) {
                ...
                IterateOverFullSlots(
                    c->common(), sizeof(SlotType), [&cb](const ctrl_t*, void* slot) {
                        ElementTypeWithConstness& element = Set::PolicyTraits::element(static_cast<SlotType*>(slot));
                        cb(element);
                    });
            }
        };
...
#undef FERMAT_SWISSTABLE_ENABLE_GENERATIONS
#undef FERMAT_SWISSTABLE_ASSERT
