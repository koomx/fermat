...
#include <fermat/hashmaps/internal/raw_hash_set_resize_impl.h>
...
#define FERMAT_SWISSTABLE_ASSERT(CONDITION) \
    assert((CONDITION) && "Try enabling sanitizers.")
            ...
            void ValidateMaxSize([[maybe_unused]] size_t size,
                ...
                FERMAT_SWISSTABLE_ASSERT(size <= MaxValidSize(key_size, slot_size));
            }
            ...
            inline void* NextSlot(void* slot, size_t slot_size, size_t i = 1) {
                return reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(slot) + slot_size * i);
            }
        ...
        namespace {
            ...
            std::pair<FindInfo, Group::NonIterableBitMaskType> find_first_non_full_group(
                ...
                auto mask = probe_till_first_non_full_group(common.control(), seq, common.capacity());
            ...
            void IterateOverFullSlotsImpl(const CommonFields& c, size_t slot_size, Fn cb) {
                ...
                if (is_half_group(cap)) {
                    ...
                    FERMAT_SWISSTABLE_ASSERT(cap <= GroupPortableImpl::kWidth && "unexpectedly large half-group capacity");
                ...
                while (remaining != 0) {
                    for (uint32_t i : GroupFullEmptyOrDeleted(ctrl).MaskFull()) {
                        FERMAT_SWISSTABLE_ASSERT(IsFull(ctrl[i]) && "hash table was modified unexpectedly");
                        cb(ctrl + i, SlotAddress(slot, i, slot_size));
                        --remaining;
                    }
                    ctrl += Group::kWidth;
                    ...
                    FERMAT_SWISSTABLE_ASSERT(
                        (remaining == 0 || *(ctrl - 1) != ctrl_t::kSentinel) && "hash table was modified unexpectedly");
                }
                // NOTE: erasure of the current element is allowed in callback for
                // turbo::erase_if specialization. So we use `>=`.
                FERMAT_SWISSTABLE_ASSERT(original_size_for_assert >= c.size() && "hash table was modified unexpectedly");
            }
            ...
            constexpr uint64_t GetPackedIncrement(uint64_t lower_bound_increment,
                ...
                return (lower_bound_increment << GrowthInfoAccessor::kLowerBoundShift) + overflow_increment;
            }
            ...
            constexpr uint64_t GetOverflowGrowthLeftFromPacked(
                ...
                constexpr uint64_t kFullGrowthMask = (uint64_t { 1 } << GrowthInfoAccessor::kLowerBoundShift) - 1;
            ...
            constexpr GrowthInfoLowerBound GetGrowthInfoLowerBoundFromPacked(
                uint64_t packed_full_growth_info) {
                return GrowthInfoLowerBound(packed_full_growth_info >> GrowthInfoAccessor::kLowerBoundShift);
            }
            ...
            uint64_t GetGrowthLeftTotalBigCapacity(void* full_growth_info) {
                uint64_t packed_full_growth_left = turbo::little_endian::Load64(full_growth_info);
                return GetOverflowGrowthLeftFromPacked(packed_full_growth_left) + GetGrowthLeftLowerBoundFromPacked(packed_full_growth_left);
            }
        ...
        void CommonFields::AssertNotDebugCapacityImpl() const {
            ...
            assert(!cap.IsReentrance() && "Reentrant container access during element construction/destruction "
                                          "is not allowed.");
            if (cap.IsDestroyed()) {
        ...
        void GrowthInfoAccessor::InitGrowthLeftNoDeleted(size_t growth_left,
            ...
            if (capacity <= GrowthInfoLowerBound::kMaxGrowthLeftLowerBound) {
                *growth_info_lower_bound_ = static_cast<uint8_t>(growth_left);
                ...
                uint64_t lower_bound = (std::min)(uint64_t { growth_left },
                    GrowthInfoLowerBound::kMaxGrowthLeftLowerBound);
                turbo::little_endian::Store64(
        ...
        GrowthInfoAccessor::RebalanceGrowthLeftLowerBoundLargeCapacity() {
            ...
            uint64_t packed_full_growth_info = turbo::little_endian::Load64(full_growth_info);
            uint64_t overflow_growth_left = GetOverflowGrowthLeftFromPacked(packed_full_growth_info);
            uint64_t lower_bound_growth_left = GetGrowthLeftLowerBoundFromPacked(packed_full_growth_info);
            uint64_t overflow_to_lower_bound_size = (std::min)(overflow_growth_left,
                GrowthInfoLowerBound::kMaxGrowthLeftLowerBound - lower_bound_growth_left);
            packed_full_growth_info += GetRebalanceIncrement(overflow_to_lower_bound_size);
            turbo::little_endian::Store64(full_growth_info, packed_full_growth_info);
            auto result = GetGrowthInfoLowerBoundFromPacked(packed_full_growth_info);
            FERMAT_SWISSTABLE_ASSERT(result.HasNoDeleted() == GetGrowthInfoLowerBound().HasNoDeleted());
            FERMAT_SWISSTABLE_ASSERT(
                (result.GetGrowthLeft() > 0 || GetGrowthLeftTotalBigCapacity(full_growth_info_ptr()) == 0) && "rebalance may return 0 only if we have absolutely no growth left");
            return result;
        }

        void GrowthInfoAccessor::OverwriteFullAsEmpty() {
            if (GetGrowthLeftLowerBound() < GrowthInfoLowerBound::kMaxGrowthLeftLowerBound) {
                ...
                turbo::little_endian::Store64(
                    full_growth_info, turbo::little_endian::Load64(full_growth_info) + kIncrement);
            }
        }

        void ConvertDeletedToEmptyAndFullToDeleted(ctrl_t* ctrl, size_t capacity) {
            FERMAT_SWISSTABLE_ASSERT(ctrl[capacity] == ctrl_t::kSentinel);
        ...
        namespace {
            ...
            inline void SetCtrlNoSanitizeImpl(const CommonFields& c, size_t i, ctrl_t h) {
                FERMAT_SWISSTABLE_ASSERT(i < c.capacity());
            ...
            inline void SetCtrl(const CommonFields& c, size_t i, ctrl_t h,
                size_t slot_size) {
                FERMAT_SWISSTABLE_ASSERT(!c.is_small());
            ...
            inline void SetCtrlInSingleGroupTable(const CommonFields& c, size_t i, ctrl_t h,
                ...
                const size_t cap = c.capacity();
                FERMAT_SWISSTABLE_ASSERT(!c.is_small());
                FERMAT_SWISSTABLE_ASSERT(is_single_group(cap));
                DoSanitizeOnSetCtrl(c, i, h, slot_size);
                ctrl_t* ctrl = c.control();
                ctrl[i] = h;
                ctrl[i + cap + 1] = h;
            ...
            inline void SetCtrlInLargeTable(const CommonFields& c, size_t i, ctrl_t h,
                size_t slot_size) {
                FERMAT_SWISSTABLE_ASSERT(c.capacity() >= Group::kWidth - 1);
                DoSanitizeOnSetCtrl(c, i, h, slot_size);
                ctrl_t* ctrl = c.control();
                ctrl[i] = h;
            ...
            void* DropDeletesWithoutResizeAndPrepareInsert(
                ...
                const size_t capacity = common.capacity();
                FERMAT_SWISSTABLE_ASSERT(IsValidCapacity(capacity));
                FERMAT_SWISSTABLE_ASSERT(!is_single_group(capacity));
                ...
                for (size_t i = 0; i != capacity;
                    ...
                    FERMAT_SWISSTABLE_ASSERT(slot_ptr == SlotAddress(slot_array, i, slot_size));
                    if (IsEmpty(ctrl[i])) {
                        tmp_space_id = i;
                        continue;
                    }
                    ...
                    if (IsEmpty(ctrl[new_i])) {
                        ...
                        FERMAT_SWISSTABLE_ASSERT(IsDeleted(ctrl[new_i]));
                        SetCtrlInLargeTable(common, new_i, h2, slot_size);
            ...
            void ResetCtrl(CommonFields& common, size_t slot_size,
                ...
                FERMAT_SWISSTABLE_ASSERT(IsCapacityValidForBlockedElements(common.capacity()) || blocked_element_count == 0);
                const size_t capacity = common.capacity();
                ctrl_t* ctrl = common.control();
            ...
            KUMO_ATTRIBUTE_ALWAYS_INLINE inline void InitializeThreeElementsControlBytes(
                ...
                static_assert(SooSlotIndex() == 1);
                FERMAT_SWISSTABLE_ASSERT(new_offset == 0 || new_offset == 2);

                static constexpr uint64_t kEmptyXorSentinel = static_cast<uint8_t>(ctrl_t::kEmpty) ^ static_cast<uint8_t>(ctrl_t::kSentinel);
                static constexpr uint64_t kEmpty64 = static_cast<uint8_t>(ctrl_t::kEmpty);
                static constexpr size_t kMirroredSooSlotIndex = SooSlotIndex() + kNewCapacity + 1;
                // The first 8 bytes, where SOO slot original and mirrored positions are
                // replaced with 0.
                // Result will look like: E0ESE0EE
                static constexpr uint64_t kFirstCtrlBytesWithZeroes = k8EmptyBytes ^ (kEmpty64 << (8 * SooSlotIndex())) ^ (kEmptyXorSentinel << (8 * kNewCapacity)) ^ (kEmpty64 << (8 * kMirroredSooSlotIndex));

                const uint64_t soo_h2 = static_cast<uint64_t>(orig_h2);
                const uint64_t new_h2_xor_empty = static_cast<uint64_t>(new_h2 ^ static_cast<uint8_t>(ctrl_t::kEmpty));
                // Fill the original and mirrored bytes for SOO slot.
                // Result will look like:
                // EHESEHEE
                // Where H = soo_h2, E = kEmpty, S = kSentinel.
                uint64_t first_ctrl_bytes = ((soo_h2 << (8 * SooSlotIndex())) | kFirstCtrlBytesWithZeroes) | (soo_h2 << (8 * kMirroredSooSlotIndex));
            ...
            void ClearBackingArrayNoReuse(CommonFields& c,
                ...
                void* alloc) {
                FERMAT_SWISSTABLE_ASSERT(c.capacity() > policy.soo_capacity());
        ...
        void EraseMetaOnlyLarge(CommonFields& c, size_t index, size_t slot_size) {
            FERMAT_SWISSTABLE_ASSERT(!c.is_small());
            FERMAT_SWISSTABLE_ASSERT(IsFull(c.control()[index]) && "erasing a dangling iterator");
            c.decrement_size();
        ...
        void ClearBackingArray(CommonFields& c,
            ...
            bool reuse) {
            FERMAT_SWISSTABLE_ASSERT(c.capacity() > kMaxSmallCapacity);
            if (reuse) {
                ...
                FERMAT_SWISSTABLE_ASSERT(c.blocked_element_count() == blocked_element_count);
            } else {
        ...
        void DestroySlots(CommonFields& c, size_t slot_size,
            ...
            FERMAT_SWISSTABLE_ASSERT(!c.is_small());
            FERMAT_SWISSTABLE_ASSERT(destroy_slot != nullptr);
        ...
        void Clear(CommonFields& c, const PolicyFunctions& __restrict policy,
            ...
            if (SwisstableGenerationsEnabled() && c.maybe_invalid_capacity().IsMovedFrom()) {
        ...
        void DestructSoo(CommonFields& c, size_t slot_size, size_t slot_align,
            ...
            void* alloc) {
            FERMAT_SWISSTABLE_ASSERT(!c.is_small() || !c.empty());
            if (c.is_small()) {
                FERMAT_SWISSTABLE_ASSERT(destroy_slot != nullptr);
        ...
        void DestructNonSoo(CommonFields& c, size_t slot_size, size_t slot_align,
            DestroySlotFn destroy_slot, DeallocBackingArrayFn dealloc,
            void* alloc) {
            FERMAT_SWISSTABLE_ASSERT(c.capacity() > 0);
            if (destroy_slot != nullptr) {
                if (c.is_small()) {
                    if (!c.empty()) {
        ...
        namespace {
            ...
            BackingArrayPtrs AllocBackingArray(CommonFields& common,
                ...
                char* mem = static_cast<char*>(
                    KUMO_LIKELY(
                        policy.alloc == (&AllocateBackingArray<kDefaultAlignment, std::allocator<char>>))
            ...
            void ResizeEmptyNonAllocatedTableImpl(CommonFields& common,
                ...
                size_t new_capacity,
                size_t blocked_element_count) {
                FERMAT_SWISSTABLE_ASSERT(IsValidCapacity(new_capacity));
                FERMAT_SWISSTABLE_ASSERT(new_capacity > policy.soo_capacity());
                FERMAT_SWISSTABLE_ASSERT(common.capacity() == policy.soo_capacity());
                FERMAT_SWISSTABLE_ASSERT(common.empty());
                const size_t slot_size = policy.slot_size;
            ...
            void InsertOldSooSlotAndInitializeControlBytes(
                ...
                void* new_slots) {
                FERMAT_SWISSTABLE_ASSERT(c.size() == policy.soo_capacity());
                FERMAT_SWISSTABLE_ASSERT(policy.soo_enabled);
            ...
            void AssertFullSoo([[maybe_unused]] CommonFields& common,
                ...
                FERMAT_SWISSTABLE_ASSERT(common.size() == policy.soo_capacity());
            }
            ...
            void GrowIntoSingleGroupShuffleControlBytes(ctrl_t* __restrict old_ctrl,
                ...
                size_t new_capacity) {
                FERMAT_SWISSTABLE_ASSERT(is_single_group(new_capacity));
                ...
                if (KUMO_UNLIKELY(old_blocked_element_count > 0)) {
                    ...
                    static constexpr uint64_t kAllBytesEmptyXorSentinel = kEmptyXorSentinel * uint64_t { 0x0101010101010101 };
                ...
                if (Group::kWidth == 8) {
                    ...
                    FERMAT_SWISSTABLE_ASSERT(old_capacity < 8 && "old_capacity is too large for group size 8");
                    turbo::little_endian::Store64(new_ctrl, copied_bytes);

                    static constexpr uint64_t kSentinal64 = static_cast<uint8_t>(ctrl_t::kSentinel);
                ...
                FERMAT_SWISSTABLE_ASSERT(Group::kWidth == 16); // NOLINT(misc-static-assert)
            ...
            KUMO_ATTRIBUTE_NOINLINE size_t DecodeAndInsertImpl(
                ...
                for (; start < end; ++start) {
                    ...
                    const size_t new_i = target.offset;
                    FERMAT_SWISSTABLE_ASSERT(old_index < new_capacity.capacity() / 2);
                    FERMAT_SWISSTABLE_ASSERT(new_i < new_capacity.capacity());
            ...
            constexpr size_t kMaxLocalBufferOldCapacity = kProbedElementsBufferSize / sizeof(ProbedItem4Bytes) - 1;
            ...
            static_assert(NextCapacity(kMaxLocalBufferNewCapacity) <= ProbedItem4Bytes::kMaxNewCapacity);
            ...
            public:
                ...
                explicit ProbedItemEncoder(ctrl_t* control)
                    : control_(control) { }
                ...
                void EncodeItem(ProbedItem item) {
                    ...
                    FERMAT_SWISSTABLE_ASSERT(pos_ < end_);
                    *pos_ = item;
                    ++pos_;
                }
                ...
                size_t DecodeAndInsertToTable(CommonFields& common,
                    ...
                    total_probe_length += DecodeAndInsertToTableOverflow(common, policy, old_slots);
                    return total_probe_length;
                }
            ...
            private:
                ...
                ProbedItem* OverflowBufferStart() const {
                    FERMAT_SWISSTABLE_ASSERT(!kGuaranteedFitToBuffer && "OverflowBufferStart should not be called when "
                                                                       "kGuaranteedFitToBuffer is true.");
                    // We reuse GrowthInfo memory as well.
                    return AlignToNextItem(
                        control_ - MetadataBeforeControlSize(NextCapacity(kMaxLocalBufferOldCapacity)));
                }
                ...
                KUMO_ATTRIBUTE_NOINLINE void ProcessEncodeWithOverflow(ProbedItem item) {
                    ...
                    // buffer.
                    if (KUMO_UNLIKELY(marked_elements_starting_position_ != kNoMarkedElementsSentinel)) {
                ...
                KUMO_ATTRIBUTE_NOINLINE size_t DecodeAndInsertToTableOverflow(
                    CommonFields& common, const PolicyFunctions& __restrict policy,
                    void* old_slots) const {
                    FERMAT_SWISSTABLE_ASSERT(local_buffer_full_ && "must not be called when local buffer is not full");
                    size_t total_probe_length = DecodeAndInsertImpl(
                        common, policy, OverflowBufferStart(), pos_, old_slots);
                    if (KUMO_LIKELY(marked_elements_starting_position_ == kNoMarkedElementsSentinel)) {
                        return total_probe_length;
                    }
                    total_probe_length += ProcessProbedMarkedElements(common, policy, control_, old_slots,
                        marked_elements_starting_position_);
                    return total_probe_length;
                }

                static constexpr size_t kBufferSize = kProbedElementsBufferSize / sizeof(ProbedItem);
            ...
            size_t GrowToNextCapacity(CommonFields& common,
                ...
                FERMAT_SWISSTABLE_ASSERT(common.capacity() <= ProbedItem::kMaxNewCapacity);
            ...
            size_t GrowToNextCapacityThatFitsInLocalBuffer(
                CommonFields& common, const PolicyFunctions& __restrict policy,
                ctrl_t* old_ctrl, void* old_slots) {
                FERMAT_SWISSTABLE_ASSERT(common.capacity() <= kMaxLocalBufferNewCapacity);
            ...
            void* Grow1To3AndPrepareInsert(CommonFields& common,
                ...
                FERMAT_SWISSTABLE_ASSERT(common.capacity() == 1);
                FERMAT_SWISSTABLE_ASSERT(!common.empty());
                FERMAT_SWISSTABLE_ASSERT(!policy.soo_enabled);
                // 1-element tables can't have any blocked elements.
                FERMAT_SWISSTABLE_ASSERT(common.blocked_element_count() == 0);
                ...
                size_t orig_hash = policy.hash_slot(policy.hash_fn(common), old_slots, common.seed().seed());
                ...
                policy.dealloc(alloc, kOldCapacity,
                    ...
                    /*blocked_element_count=*/0);
                PrepareInsertCommon(common);
                FERMAT_SWISSTABLE_ASSERT(common.size() == 2);
            ...
            void* GrowToNextCapacityAndPrepareInsert(
                ...
                FERMAT_SWISSTABLE_ASSERT(old_capacity > policy.soo_capacity());
                FERMAT_SWISSTABLE_ASSERT(!IsSmallCapacity(old_capacity));
                ...
                if (KUMO_LIKELY(is_single_group(new_capacity))) {
                    ...
                    offset = SingleGroupTableH1(new_hash, common.seed()) & 1 ? 0 : new_capacity - 1;

                    FERMAT_SWISSTABLE_ASSERT(IsEmpty(new_ctrl[offset]));
        ...
        void* PrepareInsertSmallNonSoo(CommonFields& common,
            ...
            FERMAT_SWISSTABLE_ASSERT(common.capacity() == 0);
        ...
        namespace {
            ...
            void* RehashOrGrowToNextCapacityAndPrepareInsert(
                ...
                if (common.size() * uint64_t { 32 } <= (cap - kMaxBlockedElementsForLargeTables) * uint64_t { 25 }) {
            ...
            void* PrepareInsertLargeSlow(CommonFields& common,
                ...
                const size_t cap = common.capacity();
                KUMO_ASSUME(cap > kMaxSmallCapacity);
                GrowthInfoLowerBound growth_info_lower_bound = growth_info.RebalanceGrowthLeftLowerBound(cap);
                if (KUMO_LIKELY(
            ...
            KUMO_ATTRIBUTE_NOINLINE void ReserveAllocatedTable(
                ...
                if (cap == policy.soo_capacity()) {
                    ...
                    FERMAT_SWISSTABLE_ASSERT(cap > policy.soo_capacity());
        ...
        void ResizeAllocatedTableWithSeedChange(
            ...
            FERMAT_SWISSTABLE_ASSERT(old_capacity > 0);
        ...
        void Rehash(CommonFields& common, const PolicyFunctions& __restrict policy,
            ...
            if (n == 0) {
                ...
                if (common.size() <= policy.soo_capacity()) {
                    FERMAT_SWISSTABLE_ASSERT(slot_size <= sizeof(HeapOrSoo));
                    FERMAT_SWISSTABLE_ASSERT(policy.slot_align <= alignof(HeapOrSoo));
                    HeapOrSoo tmp_slot;
            ...
            const size_t new_capacity = NormalizeCapacity(n | SizeToCapacity(common.size()));
        ...
        void Copy(CommonFields& common, const PolicyFunctions& __restrict policy,
            ...
            const size_t blocked_element_count = common.blocked_element_count();
            FERMAT_SWISSTABLE_ASSERT(other.capacity() > soo_capacity);
            const size_t cap = common.capacity();
            FERMAT_SWISSTABLE_ASSERT(cap > soo_capacity);
            KUMO_ASSUME(cap > kMaxSmallCapacity);
            size_t offset = cap;
            const void* hash_fn = policy.hash_fn(common);
            auto hasher = policy.hash_slot;
        ...
        void ReserveTableToFitNewSize(CommonFields& common,
            const PolicyFunctions& __restrict policy,
            size_t new_size) {
            new_size = std::min(new_size, MaxValidSize(policy.key_size, policy.slot_size));
            common.reset_reserved_growth(new_size);
            common.set_reservation_size(new_size);
            FERMAT_SWISSTABLE_ASSERT(new_size > policy.soo_capacity());
            const size_t cap = common.capacity();
            ...
            FERMAT_SWISSTABLE_ASSERT(!common.empty() || cap > policy.soo_capacity());
            FERMAT_SWISSTABLE_ASSERT(cap > 0);
            const size_t max_size_before_growth = IsSmallCapacity(cap)
                ? cap
        ...
        void* PrepareInsertLarge(CommonFields& common,
            ...
            FERMAT_SWISSTABLE_ASSERT(!SwisstableGenerationsEnabled());
        ...
        void* PrepareInsertLargeGenerationsEnabled(
            ...
            if (growth_left > 0 && common.should_rehash_for_bug_detection_on_insert()) {
                ...
                std::tie(target_group, mask_empty) = find_first_non_full_group(common, hash);
            }
            return PrepareInsertLargeImpl(common, policy, hash, mask_empty, target_group);
        }
