...
namespace fermat {
    ...
    template <typename Key, typename Value,
        typename KeyHash = typename fermat::flat_hash_set<Key>::hasher,
        typename KeyEq =
            typename fermat::flat_hash_set<Key, KeyHash>::key_equal,
    ...
    public:
        ...
        mapped_type& at(const key_arg<K>& key) {
            ...
            if (KUMO_UNLIKELY(it == end())) {
                turbo::ThrowStdOutOfRange("fermat::linked_hash_map::at");
            }
            return it->second;
        }
    ...
    private:
        ...
        std::pair<iterator, bool> InsertOrAssignInternal(K&& k, V&& v) {
            auto [it, inserted] = LazyEmplaceInternal(std::forward<K>(k), std::forward<V>(v));
            if (!inserted) {
        ...
        std::pair<iterator, bool> LazyEmplaceInternal(K&& key, Args&&... args) {
            ...
            auto set_iter = set_.lazy_emplace(
                key, [this, &constructed, &key, &args...](const auto& ctor) {
                    auto list_iter = list_.emplace(list_.end(), std::piecewise_construct,
