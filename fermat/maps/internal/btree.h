...
#ifdef FERMAT_BTREE_ENABLE_GENERATIONS
#error FERMAT_BTREE_ENABLE_GENERATIONS cannot be directly set
...
#define FERMAT_BTREE_ENABLE_GENERATIONS
#endif
        ...
        using btree_is_key_compare_to = std::is_convertible<compare_result_t<Compare, T, T>, turbo::weak_ordering>;
        ...
        struct key_compare_adapter {
            ...
            public:
                using Base::Base;
                checked_compare(Compare cmp)
                    : Base(std::move(cmp)) { } // NOLINT
        ...
        constexpr bool compare_has_valid_result_type() {
            ...
            return std::is_same_v<compare_result_type, bool> || std::is_convertible_v<compare_result_type, turbo::weak_ordering>;
        }
        ...
        struct common_params : base_internal::common_policy_traits<SlotPolicy> {
            ...
            // for users.
            using key_compare = std::conditional_t<!compare_has_valid_result_type<Compare, Key>(),
                Compare,
                typename key_compare_adapter<Compare, Key>::type>;

            static constexpr bool kIsKeyCompareStringAdapted = std::is_same_v<key_compare, StringBtreeDefaultLess> || std::is_same_v<key_compare, StringBtreeDefaultGreater>;
            static constexpr bool kIsKeyCompareTransparent = base_internal::IsTransparent<original_key_compare>::value || kIsKeyCompareStringAdapted;
            ...
            using value_compare = std::conditional_t<IsMap,
            ...
            constexpr static bool can_have_multiple_equivalent_keys() {
                return IsMulti || (base_internal::IsTransparent<key_compare>::value && !std::is_same_v<LookupKey, Key> && !kIsKeyCompareStringAdapted);
            }
            ...
            using node_count_type = std::conditional_t<(kNodeSlotSpace / sizeof(slot_type) > (std::numeric_limits<uint8_t>::max)()),
        ...
        enum class MatchKind : uint8_t { kEq,
            kNe };
        ...
        public:
            ...
            //   btree_node *parent;
            //
            //   // When FERMAT_BTREE_ENABLE_GENERATIONS is defined, we also have a
        ...
        private:
            using layout_type = fermat::base_internal::Layout<btree_node*, uint32_t, field_type,
            ...
            constexpr static size_type NodeTargetSlots(const size_type begin,
                ...
                return begin == end ? begin
                    : SizeWithNSlots((begin + end) / 2 + 1) > params_type::kTargetNodeSize
        ...
        public:
            ...
            bool is_ordered_correctly(field_type i, const Compare& comp) const {
                if (std::is_base_of_v<BtreeTestOnlyCheckedCompareOptOutBase, Compare> || params_type::kIsKeyCompareStringAdapted) {
                    return true;
                }

                const auto compare = [&](field_type a, field_type b) {
                    const turbo::weak_ordering cmp = turbo::compare_internal::do_three_way_comparison(comp, key(a), key(b));
        ...
        public:
            ...
            template <
                ...
                std::enable_if_t<std::is_same_v<btree_iterator<N, R, P>, iterator> && std::is_same_v<btree_iterator, const_iterator>,
                    int> = 0>
        ...
        private:
            ...
            template <typename N, typename R, typename P,
                std::enable_if_t<
                    std::is_same_v<btree_iterator<N, R, P>, const_iterator> && std::is_same_v<btree_iterator, iterator>,
                    int> = 0>
            ...
            bool Equals(const const_iterator other) const {
                turbo::base_internal::HardeningAssert(
                    ((node_ == nullptr && other.node_ == nullptr) || (node_ != nullptr && other.node_ != nullptr)) && "Comparing default-constructed iterator with "
        ...
        public:
            ...
            using node_handle_type = base_internal::node_handle<Params, Params, allocator_type>;
        ...
        public:
            ...
            bool compare_keys(const K1& a, const K2& b) const {
                return turbo::compare_internal::compare_result_as_less_than(key_comp()(a, b));
            }
        ...
        void btree<P>::insert_iterator_unique(InputIterator b, InputIterator e, char) {
            for (; b != e; ++b) {
                ...
                auto node_handle = base_internal::CommonAccess::Construct<node_handle_type>(get_allocator(), *b);
                slot_type* slot = base_internal::CommonAccess::GetSlot(node_handle);
        ...
        inline auto btree<P>::internal_emplace(iterator iter, Args&&... args)
            ...
            (void)replaced_node;
#if KUMO_HAVE_ADDRESS_SANITIZER || KUMO_HAVE_HWADDRESS_SANITIZER
            if (!replaced_node) {
                assert(iter.node_->is_leaf());
...
#undef FERMAT_BTREE_ENABLE_GENERATIONS
