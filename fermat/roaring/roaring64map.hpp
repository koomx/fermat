 ...
 */
#ifndef INCLUDE_FERMAT_ROARING_64_MAP_HH_
#define INCLUDE_FERMAT_ROARING_64_MAP_HH_
...
class Roaring64Map {
   ...
   public:
    ...
    static Roaring64Map readSafe(const char *buf, size_t maxbytes) {
        if (maxbytes < sizeof(uint64_t)) {
            FERMAT_ROARING_TERMINATE("ran out of bytes");
        }
        Roaring64Map result;
        if (maxbytes < sizeof(uint64_t)) {
            FERMAT_ROARING_TERMINATE("ran out of bytes");
        ...
        for (uint64_t lcv = 0; lcv < map_size; lcv++) {
            ...
            if (needed_bytes == 0) {
                FERMAT_ROARING_TERMINATE("invalid serialized data");
            }
   ...
   private:
    ...
    roarings_t::iterator ensureRangePopulated(uint32_t start_high,
                                              uint32_t end_high) {
        if (start_high > end_high) {
            FERMAT_ROARING_TERMINATE("Logic error: start_high > end_high");
        }
...
class Roaring64MapSetBitBiDirectionalIterator {
   public:
    ...
    CFERMAT_ROARING_DEPRECATED bool move(const value_type &x) {
...
#endif /* INCLUDE_FERMAT_ROARING_64_MAP_HH_ */
