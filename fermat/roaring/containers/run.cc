...
#include <fermat/roaring/containers/run.h>
...
void run_container_andnot(const run_container_t *src_1,
    ...
    while ((rlepos1 < src_1->n_runs) && (rlepos2 < src_2->n_runs)) {
        if (end <= start2) {
            ...
            dst->runs[dst->n_runs++] =
                CFERMAT_ROARING_MAKE_RLE16(start, end - start - 1);
            rlepos1++;
            ...
            if (start < start2) {
                dst->runs[dst->n_runs++] =
                    CFERMAT_ROARING_MAKE_RLE16(start, start2 - start - 1);
            }
            if (end2 < end) {
                start = end2;
            } else {
    ...
    if (rlepos1 < src_1->n_runs) {
        dst->runs[dst->n_runs++] = CFERMAT_ROARING_MAKE_RLE16(start, end - start - 1);
        rlepos1++;
        if (rlepos1 < src_1->n_runs) {
...
void run_container_smart_append_exclusive(run_container_t *src,
    ...
    if (!src->n_runs ||
        ...
        *appended_last_run = CFERMAT_ROARING_MAKE_RLE16(start, length);
        src->n_runs++;
        return;
    }
    if (old_end == start) {
    ...
    if (start == last_run->value) {
        // wipe out previous
        if (new_end < old_end) {
            *last_run = CFERMAT_ROARING_MAKE_RLE16(new_end, old_end - new_end - 1);
            return;
        } else if (new_end > old_end) {
            *last_run = CFERMAT_ROARING_MAKE_RLE16(old_end, new_end - old_end - 1);
            return;
        } else {
            src->n_runs--;
            return;
    ...
    if (new_end < old_end) {
        *appended_last_run =
            CFERMAT_ROARING_MAKE_RLE16(new_end, old_end - new_end - 1);
        src->n_runs++;
    } else if (new_end > old_end) {
        *appended_last_run =
            CFERMAT_ROARING_MAKE_RLE16(old_end, new_end - old_end - 1);
        src->n_runs++;
    }
}
...
#define CFERMAT_ROARING_ENABLE_AVX512_RUN_CONTAINER_CARDINALITY 0

#if defined(CFERMAT_ROARING_IS_X64) && CFERMAT_ROARING_COMPILER_SUPPORTS_AVX512

#if CFERMAT_ROARING_ENABLE_AVX512_RUN_CONTAINER_CARDINALITY
...
#endif  // CFERMAT_ROARING_ENABLE_AVX512_RUN_CONTAINER_CARDINALITY
...
int run_container_cardinality(const run_container_t *run) {
    ...
    CFERMAT_ROARING_ENABLE_AVX512_RUN_CONTAINER_CARDINALITY
