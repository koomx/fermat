...
#include <fermat/roaring/roaring_types.h>  // roaring_iterator
...
#include <fermat/roaring/array_util.h>  // binarySearch()/memequals() for inlining
#include <fermat/roaring/containers/container_defs.h>  // container_t, perfparameters
...
#ifdef __cplusplus
#define CFERMAT_ROARING_MAKE_RLE16(val, len) \
...
static inline void run_container_append_value(run_container_t *run,
    ...
    if (val > previousend + 1) {  // we add a new one
        *previousrl = CFERMAT_ROARING_MAKE_RLE16(val, 0);
...
static inline rle16_t run_container_append_value_first(run_container_t *run,
                                                       uint16_t val) {
    rle16_t newrle = CFERMAT_ROARING_MAKE_RLE16(val, 0);
