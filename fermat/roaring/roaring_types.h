...
#ifndef FERMAT_ROARING_TYPES_H
#define FERMAT_ROARING_TYPES_H
 ...
 * with other projects.  So roaring.h uses FERMAT_ROARING_CONTAINER_T, while internal
 * code #undefs that after declaring `typedef FERMAT_ROARING_CONTAINER_T container_t;`
 */
#if defined(__cplusplus)
extern "C++" {
struct container_s {};
}
#define FERMAT_ROARING_CONTAINER_T ::fermat::roaring::api::container_s
#else
#define FERMAT_ROARING_CONTAINER_T void  // no compile-time checking
#endif

#define FERMAT_ROARING_FLAG_COW UINT8_C(0x1)
#define FERMAT_ROARING_FLAG_FROZEN UINT8_C(0x2)
...
typedef struct roaring_array_s {
    ...
    FERMAT_ROARING_CONTAINER_T **containers;  // Use container_t in non-API files!
    uint16_t *keys;
    uint8_t *typecodes;
    uint8_t flags;
} roaring_array_t;
