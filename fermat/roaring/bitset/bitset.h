 ...
 */
#ifndef CFERMAT_ROARING_CBITSET_BITSET_H
#define CFERMAT_ROARING_CBITSET_BITSET_H

// For compatibility with MSVC with the use of `restrict`
#ifdef __cplusplus
#define CFERMAT_ROARING_CBITSET_RESTRICT
#elif (__STDC_VERSION__ >= 199901L) || \
    (defined(__GNUC__) && defined(__STDC_VERSION__))
#define CFERMAT_ROARING_CBITSET_RESTRICT restrict
#else
#define CFERMAT_ROARING_CBITSET_RESTRICT
#endif
...
struct bitset_s {
    uint64_t *CFERMAT_ROARING_CBITSET_RESTRICT array;
...
bool bitset_inplace_union(bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b1,
                          const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b2);

/* report the size of the union (without materializing it) */
size_t bitset_union_count(const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b1,
                          const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b2);

/* compute the intersection in-place (to b1), to generate a new bitset first
 * call bitset_copy */
void bitset_inplace_intersection(bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b1,
                                 const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b2);

/* report the size of the intersection (without materializing it) */
size_t bitset_intersection_count(const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b1,
                                 const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b2);

/* returns true if the bitsets contain no common elements */
bool bitsets_disjoint(const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b1,
                      const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b2);

/* returns true if the bitsets contain any common elements */
bool bitsets_intersect(const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b1,
                       const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b2);

/* returns true if b1 contains all of the set bits of b2 */
bool bitset_contains_all(const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b1,
                         const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b2);

/* compute the difference in-place (to b1), to generate a new bitset first call
 * bitset_copy */
void bitset_inplace_difference(bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b1,
                               const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b2);

/* compute the size of the difference */
size_t bitset_difference_count(const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b1,
                               const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b2);
...
bool bitset_inplace_symmetric_difference(
    bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b1,
    const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b2);

/* compute the size of the symmetric difference  */
size_t bitset_symmetric_difference_count(
    const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b1,
    const bitset_t *CFERMAT_ROARING_CBITSET_RESTRICT b2);
