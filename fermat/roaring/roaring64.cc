...
// containers.h last to avoid conflict with FERMAT_ROARING_CONTAINER_T.
#include <fermat/roaring/containers/containers.h>

#define CFERMAT_ROARING_ALIGN_BUF(buf, alignment)          \
    (char *)(((uintptr_t)(buf) + ((alignment)-1)) & \
             (ptrdiff_t)(~((alignment)-1)))

#define CFERMAT_ROARING_BITSET_ALIGNMENT 64
...
static inline bool is_frozen64(const roaring64_bitmap_t *r) {
    return r->flags & FERMAT_ROARING_FLAG_FROZEN;
}
...
void roaring64_bitmap_add_many(roaring64_bitmap_t *r, size_t n_args,
    ...
    roaring64_bulk_context_t context = CFERMAT_ROARING_ZERO_INITIALIZER;
...
size_t roaring64_bitmap_frozen_size_in_bytes(const roaring64_bitmap_t *r) {
    ...
    uint64_t total_sizes[4] =
        CFERMAT_ROARING_ZERO_INITIALIZER;  // Indexed by typecode.
    art_iterator_t it = art_init_iterator((art_t *)&r->art, /*first=*/true);
    while (it.value != NULL) {
        leaf_t leaf = (leaf_t)*it.value;
        uint8_t typecode = get_typecode(leaf);
    ...
    // Containers (aligned).
    size = align_size(size, CFERMAT_ROARING_BITSET_ALIGNMENT);
...
size_t roaring64_bitmap_frozen_serialize(const roaring64_bitmap_t *r,
    ...
    buf = pad_align(buf, initial_buf, CFERMAT_ROARING_BITSET_ALIGNMENT);
...
roaring64_bitmap_t *roaring64_bitmap_frozen_view(const char *buf,
    ...
    if ((uintptr_t)buf % CFERMAT_ROARING_BITSET_ALIGNMENT != 0) {
        return NULL;
    }
    ...
    r->flags |= FERMAT_ROARING_FLAG_FROZEN;
    ...
    // ART (8 byte aligned).
    buf = CFERMAT_ROARING_ALIGN_BUF(buf, 8);
    ...
    buf = CFERMAT_ROARING_ALIGN_BUF(buf, CFERMAT_ROARING_BITSET_ALIGNMENT);
    const uint64_t *bitsets = (const uint64_t *)buf;
    buf += total_sizes[BITSET_CONTAINER_TYPE];
    buf = CFERMAT_ROARING_ALIGN_BUF(buf, alignof(rle16_t));
    const rle16_t *runs = (const rle16_t *)buf;
    buf += total_sizes[RUN_CONTAINER_TYPE];
    buf = CFERMAT_ROARING_ALIGN_BUF(buf, alignof(uint16_t));
