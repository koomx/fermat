...
*/
#ifndef INCLUDE_FERMAT_ROARING_HH_
#define INCLUDE_FERMAT_ROARING_HH_
...
#if !defined(FERMAT_ROARING_EXCEPTIONS)
// We assume that if __cpp_exceptions is given a positive integer
// value, then exceptions are enabled.
#if __cpp_exceptions
#define FERMAT_ROARING_EXCEPTIONS 1
#else
#define FERMAT_ROARING_EXCEPTIONS 0
#endif
#endif

#ifndef FERMAT_ROARING_TERMINATE
#if FERMAT_ROARING_EXCEPTIONS
#define FERMAT_ROARING_TERMINATE(_s) throw std::runtime_error(_s)
#else
#define FERMAT_ROARING_TERMINATE(_s) std::terminate()
#endif
#endif

#define FERMAT_ROARING_API_NOT_IN_GLOBAL_NAMESPACE  // see remarks in roaring.h
#include <fermat/roaring/roaring.h>
#undef FERMAT_ROARING_API_NOT_IN_GLOBAL_NAMESPACE

#include <fermat/roaring/roaring_array.h>  // fermat::roaring::internal array functions used
   ...
   public:
    ...
    Roaring(const Roaring &r) : Roaring() {
        if (!api::roaring_bitmap_overwrite(&roaring, &r.roaring)) {
            FERMAT_ROARING_TERMINATE("failed roaring_bitmap_overwrite in constructor");
        }
    ...
    Roaring &operator=(const Roaring &r) {
        if (!api::roaring_bitmap_overwrite(&roaring, &r.roaring)) {
            FERMAT_ROARING_TERMINATE("failed memory alloc in assignment");
        }
        api::roaring_bitmap_set_copy_on_write(
            &roaring, api::roaring_bitmap_get_copy_on_write(&r.roaring));
        return *this;
    ...
    static Roaring read(const char *buf, bool portable = true) {
        ...
        if (r == NULL) {
            FERMAT_ROARING_TERMINATE("failed alloc while reading");
        }
        return Roaring(r);
    }
    ...
    static Roaring frozenView(const char *buf, size_t length) {
        ...
        if (s == NULL) {
            FERMAT_ROARING_TERMINATE("failed to read frozen bitmap");
        }
        Roaring r;
        r.roaring = *s;
        return r;
    ...
    static Roaring portableDeserializeFrozen(const char *buf) {
        ...
        if (s == NULL) {
            FERMAT_ROARING_TERMINATE("failed to read portable frozen bitmap");
        }
        Roaring r;
        r.roaring = *s;
        return r;
    ...
    Roaring operator&(const Roaring &o) const {
        ...
        if (r == NULL) {
            FERMAT_ROARING_TERMINATE("failed materialization in and");
        }
        return Roaring(r);
    }
    ...
    Roaring operator-(const Roaring &o) const {
        ...
        if (r == NULL) {
            FERMAT_ROARING_TERMINATE("failed materialization in andnot");
        }
        return Roaring(r);
    }
    ...
    Roaring operator|(const Roaring &o) const {
        ...
        if (r == NULL) {
            FERMAT_ROARING_TERMINATE("failed materialization in or");
        }
        return Roaring(r);
    }
    ...
    Roaring operator^(const Roaring &o) const {
        ...
        if (r == NULL) {
            FERMAT_ROARING_TERMINATE("failed materialization in xor");
        }
        return Roaring(r);
    }
    ...
    static Roaring fastunion(size_t n, const Roaring **inputs) {
        ...
        if (x == NULL) {
            FERMAT_ROARING_TERMINATE("failed memory alloc in fastunion");
        }
     ...
     */
    ~Roaring() {
        if (!(roaring.high_low_container.flags & FERMAT_ROARING_FLAG_FROZEN)) {
...
class RoaringSetBitBiDirectionalIterator final {
   public:
    ...
    CFERMAT_ROARING_DEPRECATED void equalorlarger(uint32_t val) {
...
#endif /* INCLUDE_FERMAT_ROARING_HH_ */
