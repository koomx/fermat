...
from git hash:38636b991de64d4bf322df2afaf52a6aebd8ec8b
...
Linux or macOS users might follow the following instructions if they have a recent C or C++ compiler installed and a standard utility (`wget`).
 ...
 2. Create a new file named `demo.cpp` with this content:
    ...
    #include "roaring.c"
    int main() {
        fermat::roaring::Roaring r1;
        for (uint32_t i = 100; i < 1000; i++) {
            r1.add(i);
        }
        std::cout << "cardinality = " << r1.cardinality() << std::endl;

        fermat::roaring::Roaring64Map r2;
...
for 64-bit bitmaps) and a C++ API (`roaring.hh`, `roaring64map.hpp`, and
`roaring64.hpp`). The two short programs below cover the most common operations:
...
```c
#include <fermat/roaring/roaring.h>
#include <fermat/roaring/roaring64.h>
...
The C++ classes (`fermat::roaring::Roaring`, `fermat::roaring::Roaring64Map`, and
`fermat::roaring::Roaring64`) wrap the C API and manage memory for you: the destructor
frees the bitmap, and set operations are exposed as operators (`&`, `|`, `^`,
`-`). No explicit `free` is required.

```cpp
#include <fermat/roaring/roaring.hh>
#include <fermat/roaring/roaring64map.hh>

#include <cassert>
#include <iostream>

using namespace fermat::roaring;
...
CPMAddPackage(
  ...
  GIT_TAG v2.0.4
  OPTIONS "ENABLE_FERMAT_ROARING_TESTS OFF"
)

target_link_libraries(hello fermat::roaring::roaring)
```
...
target_link_libraries(repro PUBLIC fermat::roaring::roaring)
```
...
The C++ interface is provided via the `roaring.hh` (32-bit), `roaring64map.hpp`, and `roaring64.hpp` (64-bit) headers. These offer a modern, type-safe, and convenient API for manipulating Roaring bitmaps in C++.

## Main Classes
- `fermat::roaring::Roaring` — 32-bit Roaring bitmap
- `fermat::roaring::Roaring64Map` — 64-bit Roaring bitmap (`std::map`-based)
- `fermat::roaring::Roaring64` — 64-bit Roaring bitmap (ART-based C API wrapper; experimental)
...
For 64-bit values, there are two classes. `Roaring64Map` (`roaring64map.hpp`) keys a `std::map` by the high 32 bits, each entry a 32-bit `Roaring` bitmap. `Roaring64` (`roaring64.hpp`) wraps the C API's native 64-bit bitmap, which uses an Adaptive Radix Tree with 48-bit keys and 16-bit containers.
...
```
cmake -B build -D ENABLE_FERMAT_ROARING_MICROBENCHMARKS=ON
cmake --build build
...
```
cmake -B buildnoavx512 -D FERMAT_ROARING_DISABLE_AVX512=ON -D ENABLE_FERMAT_ROARING_MICROBENCHMARKS=ON
...
```
cmake -B buildnoavx -D FERMAT_ROARING_DISABLE_AVX=ON -D ENABLE_FERMAT_ROARING_MICROBENCHMARKS=ON
...
This example assumes that CRoaring has been build and that you are linking against the corresponding library. By default, CRoaring will install its header files in a `roaring` directory. If you are working from the amalgamation script, you may add the line `#include "roaring.c"` if you are not linking against a prebuilt CRoaring library and replace `#include <fermat/roaring/roaring.h>` by `#include "roaring.h"`.
...
This example assumes that CRoaring has been build and that you are linking against the corresponding library. By default, CRoaring will install its header files in a `roaring` directory so you may need to replace `#include "roaring.hh"` by `#include <fermat/roaring/roaring.hh>`. If you are working from the amalgamation script, you may add the line `#include "roaring.c"` if you are not linking against a CRoaring prebuilt library.
...
By default all tests are built on all platforms, to skip building and running tests add `` -DENABLE_FERMAT_ROARING_TESTS=OFF `` to the command line.
...
If you are using clang or gcc and you know your target architecture,  you can set the architecture by specifying `-DFERMAT_ROARING_ARCH=arch`. For example, if you have many server but the oldest server is running the Intel `haswell` architecture, you can specify -`DFERMAT_ROARING_ARCH=haswell`. In such cases, the produced binary will be optimized for processors having the characteristics of a haswell process and may not run on older architectures. You can find out the list of valid architecture values by typing `man gcc`.

 ```
 mkdir -p build_haswell
 cd build_haswell
 cmake -DFERMAT_ROARING_ARCH=haswell ..
 cmake --build .
 ```
...
mkdir -p debug
cd debug
cmake -DCMAKE_BUILD_TYPE=Debug -DFERMAT_ROARING_SANITIZE=ON ..
ctest
```
...
- Type ``cmake -DCMAKE_GENERATOR_PLATFORM=x64 ..`` in the shell while in the ``VisualStudio`` repository. (Alternatively, if you want to build a static library, you may use the command line ``cmake -DCMAKE_GENERATOR_PLATFORM=x64 -DFERMAT_ROARING_BUILD_STATIC=ON  ..``.)
