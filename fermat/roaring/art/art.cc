...
#include <fermat/roaring/art/art.h>
#include <fermat/roaring/memory.h>
#include <fermat/roaring/portability.h>

#define CFERMAT_ROARING_ART_NULL_REF 0

#define CFERMAT_ROARING_ART_LEAF_TYPE 1
#define CFERMAT_ROARING_ART_NODE4_TYPE 2
#define CFERMAT_ROARING_ART_NODE16_TYPE 3
#define CFERMAT_ROARING_ART_NODE48_TYPE 4
#define CFERMAT_ROARING_ART_NODE256_TYPE 5

#define CFERMAT_ROARING_ART_MIN_TYPE CFERMAT_ROARING_ART_LEAF_TYPE
#define CFERMAT_ROARING_ART_MAX_TYPE CFERMAT_ROARING_ART_NODE256_TYPE

// Node48 placeholder value to indicate no child is present at this key index.
#define CFERMAT_ROARING_ART_NODE48_EMPTY_VAL 48
#define CFERMAT_ROARING_NODE48_AVAILABLE_CHILDREN_MASK ((UINT64_C(1) << 48) - 1)

#define CFERMAT_ROARING_ART_ALIGN_BUF(buf, alignment)      \
...
#define CFERMAT_ROARING_ART_ALIGN_SIZE_RELATIVE(buf_cur, buf_start, alignment) \
...
// CFERMAT_ROARING_ART_NODE48_EMPTY_VAL. Keys are naturally sorted due to direct
// indexing.
...
static art_node_t *art_deref(const art_t *art, art_ref_t ref) {
    assert(ref != CFERMAT_ROARING_ART_NULL_REF);
...
static inline uint64_t art_get_index(const art_t *art, const art_node_t *node,
    ...
    switch (typecode) {
        case CFERMAT_ROARING_ART_LEAF_TYPE:
            return (art_leaf_t *)node - (art_leaf_t *)nodes;
        case CFERMAT_ROARING_ART_NODE4_TYPE:
            return (art_node4_t *)node - (art_node4_t *)nodes;
        case CFERMAT_ROARING_ART_NODE16_TYPE:
            return (art_node16_t *)node - (art_node16_t *)nodes;
        case CFERMAT_ROARING_ART_NODE48_TYPE:
            return (art_node48_t *)node - (art_node48_t *)nodes;
        case CFERMAT_ROARING_ART_NODE256_TYPE:
...
static inline bool art_is_leaf(art_ref_t ref) {
    return art_ref_typecode(ref) == CFERMAT_ROARING_ART_LEAF_TYPE;
}
...
static art_ref_t art_leaf_create(art_t *art, const art_key_chunk_t key[],
                                 art_val_t val) {
    uint64_t index = art_allocate_index(art, CFERMAT_ROARING_ART_LEAF_TYPE);
    art_leaf_t *leaf =
        ((art_leaf_t *)art->nodes[CFERMAT_ROARING_ART_LEAF_TYPE]) + index;
    memcpy(leaf->key, key, ART_KEY_BYTES);
    leaf->val = val;
    return art_to_ref(index, CFERMAT_ROARING_ART_LEAF_TYPE);
}
...
static art_node4_t *art_node4_create(art_t *art, const art_key_chunk_t prefix[],
                                     uint8_t prefix_size) {
    uint64_t index = art_allocate_index(art, CFERMAT_ROARING_ART_NODE4_TYPE);
    art_node4_t *node =
        ((art_node4_t *)art->nodes[CFERMAT_ROARING_ART_NODE4_TYPE]) + index;
...
static inline art_ref_t art_node4_find_child(const art_node4_t *node,
    ...
    return CFERMAT_ROARING_ART_NULL_REF;
}
...
static art_ref_t art_node4_insert(art_t *art, art_node4_t *node,
    ...
    if (node->count < 4) {
        ...
        node->keys[idx] = key;
        node->count++;
        return art_get_ref(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE4_TYPE);
    }
    ...
    art_node_free(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE4_TYPE);
...
static inline art_indexed_child_t art_node4_next_child(const art_node4_t *node,
    ...
    index++;
    if (index >= node->count) {
        indexed_child.child = CFERMAT_ROARING_ART_NULL_REF;
...
static art_node16_t *art_node16_create(art_t *art,
                                       const art_key_chunk_t prefix[],
                                       uint8_t prefix_size) {
    uint64_t index = art_allocate_index(art, CFERMAT_ROARING_ART_NODE16_TYPE);
    art_node16_t *node =
        ((art_node16_t *)art->nodes[CFERMAT_ROARING_ART_NODE16_TYPE]) + index;
    art_init_inner_node(&node->base, prefix, prefix_size);
    node->count = 0;
    return node;
}
...
static art_ref_t art_node16_insert(art_t *art, art_node16_t *node,
    ...
    if (node->count < 16) {
        ...
        node->keys[idx] = key;
        node->count++;
        return art_get_ref(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE16_TYPE);
    }
    ...
    art_node_free(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE16_TYPE);
...
static inline art_ref_t art_node16_erase(art_t *art, art_node16_t *node,
    ...
    art_node_free(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE16_TYPE);
    return art_get_ref(art, (art_node_t *)new_node, CFERMAT_ROARING_ART_NODE4_TYPE);
}
...
static art_node48_t *art_node48_create(art_t *art,
                                       const art_key_chunk_t prefix[],
                                       uint8_t prefix_size) {
    uint64_t index = art_allocate_index(art, CFERMAT_ROARING_ART_NODE48_TYPE);
    art_node48_t *node =
        ((art_node48_t *)art->nodes[CFERMAT_ROARING_ART_NODE48_TYPE]) + index;
    art_init_inner_node(&node->base, prefix, prefix_size);
    node->count = 0;
    node->available_children = CFERMAT_ROARING_NODE48_AVAILABLE_CHILDREN_MASK;
    for (size_t i = 0; i < 256; ++i) {
        node->keys[i] = CFERMAT_ROARING_ART_NODE48_EMPTY_VAL;
    }
    return node;
}
...
static inline art_ref_t art_node48_find_child(const art_node48_t *node,
    ...
    if (val_idx != CFERMAT_ROARING_ART_NODE48_EMPTY_VAL) {
...
static art_ref_t art_node48_insert(art_t *art, art_node48_t *node,
    ...
    if (node->count < 48) {
        ...
        return art_get_ref(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE48_TYPE);
    }
    ...
    art_node_free(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE48_TYPE);
...
static inline art_ref_t art_node48_erase(art_t *art, art_node48_t *node,
    ...
    node->keys[key_chunk] = CFERMAT_ROARING_ART_NODE48_EMPTY_VAL;
    node->available_children |= UINT64_C(1) << val_idx;
    node->count--;
    if (node->count > 16) {
        return art_get_ref(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE48_TYPE);
    ...
    art_node_free(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE48_TYPE);
    return art_get_ref(art, (art_node_t *)new_node, CFERMAT_ROARING_ART_NODE16_TYPE);
}
...
static inline void art_node48_replace(art_node48_t *node,
                                      ...
                                      art_ref_t new_child) {
    uint8_t val_idx = node->keys[key_chunk];
    assert(val_idx != CFERMAT_ROARING_ART_NODE48_EMPTY_VAL);
...
static inline art_indexed_child_t art_node48_next_child(
    ...
    for (size_t i = index; i < 256; ++i) {
        if (node->keys[i] != CFERMAT_ROARING_ART_NODE48_EMPTY_VAL) {
            indexed_child.index = i;
...
static bool art_node48_internal_validate(const art_t *art,
    ...
    for (int i = 0; i < 256; ++i) {
        ...
        if (child_idx != CFERMAT_ROARING_ART_NODE48_EMPTY_VAL) {
            ...
            if (child == CFERMAT_ROARING_ART_NULL_REF) {
...
static art_node256_t *art_node256_create(art_t *art,
                                         const art_key_chunk_t prefix[],
                                         uint8_t prefix_size) {
    uint64_t index = art_allocate_index(art, CFERMAT_ROARING_ART_NODE256_TYPE);
    art_node256_t *node =
        ((art_node256_t *)art->nodes[CFERMAT_ROARING_ART_NODE256_TYPE]) + index;
    art_init_inner_node(&node->base, prefix, prefix_size);
    node->count = 0;
    for (size_t i = 0; i < 256; ++i) {
        node->children[i] = CFERMAT_ROARING_ART_NULL_REF;
    }
    return node;
}
...
static art_ref_t art_node256_insert(art_t *art, art_node256_t *node,
    ...
    node->count++;
    return art_get_ref(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE256_TYPE);
}

static inline art_ref_t art_node256_erase(art_t *art, art_node256_t *node,
                                          uint8_t key_chunk) {
    node->children[key_chunk] = CFERMAT_ROARING_ART_NULL_REF;
    node->count--;
    if (node->count > 48) {
        return art_get_ref(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE256_TYPE);
    }
    ...
    art_node48_t *new_node =
        art_node48_create(art, node->base.prefix, node->base.prefix_size);
    for (size_t i = 0; i < 256; ++i) {
        if (node->children[i] != CFERMAT_ROARING_ART_NULL_REF) {
            art_node48_insert(art, new_node, node->children[i], i);
        }
    }
    art_node_free(art, (art_node_t *)node, CFERMAT_ROARING_ART_NODE256_TYPE);
    return art_get_ref(art, (art_node_t *)new_node, CFERMAT_ROARING_ART_NODE48_TYPE);
}
    ...
    switch (typecode) {
        case CFERMAT_ROARING_ART_LEAF_TYPE:
            return (art_indexed_child_t){
                .child = CFERMAT_ROARING_ART_NULL_REF,
                .index = 0,
                .key_chunk = 0,
            };
        case CFERMAT_ROARING_ART_NODE4_TYPE:
...
static art_ref_t art_insert_at(art_t *art, art_ref_t ref,
    ...
    if (art_is_leaf(ref)) {
        ...
        art_ref_t new_ref = art_node_insert_leaf(
            art, (art_inner_node_t *)new_node, CFERMAT_ROARING_ART_NODE4_TYPE,
            leaf->key[depth + common_prefix], ref);
        new_ref = art_node_insert_leaf(art, (art_inner_node_t *)new_node,
                                       CFERMAT_ROARING_ART_NODE4_TYPE,
...
static art_erase_result_t art_erase_at(art_t *art, art_ref_t ref,
    ...
    result.rootmost_node = CFERMAT_ROARING_ART_NULL_REF;
    result.erased = false;
    ...
    if (art_is_leaf(ref)) {
        ...
        art_node_free(art, (art_node_t *)leaf, CFERMAT_ROARING_ART_LEAF_TYPE);
        return result;
    }
    art_inner_node_t *inner_node = (art_inner_node_t *)art_deref(art, ref);
    uint8_t common_prefix =
    ...
    inner_node = (art_inner_node_t *)art_deref(art, ref);
    if (child_result.rootmost_node == CFERMAT_ROARING_ART_NULL_REF) {
    ...
    switch (art_ref_typecode(ref)) {
        ...
        case CFERMAT_ROARING_ART_NODE48_TYPE: {
            ...
            for (uint16_t i = 0; i < 256; ++i) {
                if (node48->keys[i] != CFERMAT_ROARING_ART_NODE48_EMPTY_VAL) {
                    printf("%*s", depth, "");
                    printf("key: %02x ", i);
        ...
        case CFERMAT_ROARING_ART_NODE256_TYPE: {
            ...
            for (uint16_t i = 0; i < 256; ++i) {
                if (node256->children[i] != CFERMAT_ROARING_ART_NULL_REF) {
                    printf("%*s", depth, "");
                    printf("key: %02x ", i);
...
static void art_sort_free_lists(art_t *art) {
    for (art_typecode_t type = CFERMAT_ROARING_ART_LEAF_TYPE;
         type <= CFERMAT_ROARING_ART_NODE256_TYPE; ++type) {
        bool *free_indices =
...
static size_t art_shrink_node_arrays(art_t *art) {
    size_t freed = 0;
    for (art_typecode_t t = CFERMAT_ROARING_ART_MIN_TYPE; t <= CFERMAT_ROARING_ART_MAX_TYPE;
         ++t) {
...
void art_init_cleared(art_t *art) {
    art->root = CFERMAT_ROARING_ART_NULL_REF;
...
size_t art_shrink_to_fit(art_t *art) {
    ...
    if (art->root != CFERMAT_ROARING_ART_NULL_REF) {
...
bool art_is_empty(const art_t *art) {
    return art->root == CFERMAT_ROARING_ART_NULL_REF;
}
...
static void art_iterator_down(art_iterator_t *iterator, art_ref_t ref,
    ...
    assert(indexed_child.child != CFERMAT_ROARING_ART_NULL_REF);
...
static art_ref_t art_iterator_neighbor_child(art_iterator_t *iterator,
    ...
    if (indexed_child.child != CFERMAT_ROARING_ART_NULL_REF) {
...
bool art_iterator_move(art_iterator_t *iterator, bool forward) {
    ...
    if (neighbor_child != CFERMAT_ROARING_ART_NULL_REF) {
...
art_iterator_t art_init_iterator(art_t *art, bool first) {
    art_iterator_t iterator = CFERMAT_ROARING_ZERO_INITIALIZER;
    iterator.art = art;
    if (art->root == CFERMAT_ROARING_ART_NULL_REF) {
        return iterator;
    }
...
bool art_iterator_lower_bound(art_iterator_t *iterator,
    ...
    if (iterator->value == NULL) {
        ...
        if (root == CFERMAT_ROARING_ART_NULL_REF) {
            return false;
        }
...
void art_iterator_insert(art_iterator_t *iterator, const art_key_chunk_t *key,
    ...
    assert(iterator->art->root != CFERMAT_ROARING_ART_NULL_REF);
    iterator->frame = 0;
    iterator->depth = 0;
...
bool art_iterator_erase(art_iterator_t *iterator, art_val_t *erased_val) {
    ...
    *erased_val = *iterator->value;
    ...
    if (!went_up) {
        ...
        iterator->art->root = CFERMAT_ROARING_ART_NULL_REF;
...
static bool art_internal_validate_at(const art_t *art, art_ref_t ref,
                                     art_internal_validate_t validator) {
    if (ref == CFERMAT_ROARING_ART_NULL_REF) {
...
bool art_internal_validate(const art_t *art, const char **reason,
    ...
    *reason = NULL;
    ...
    art_internal_validate_t validator = {
        ...
        .context = context,
        .depth = 0,
        .current_key = CFERMAT_ROARING_ZERO_INITIALIZER,
    };
    for (art_typecode_t type = CFERMAT_ROARING_ART_LEAF_TYPE;
         type <= CFERMAT_ROARING_ART_NODE256_TYPE; ++type) {
...
CFERMAT_ROARING_STATIC_ASSERT(alignof(art_leaf_t) == alignof(art_node4_t),
                       "Serialization assumes node type alignment is equal");
CFERMAT_ROARING_STATIC_ASSERT(alignof(art_leaf_t) == alignof(art_node16_t),
                       "Serialization assumes node type alignment is equal");
CFERMAT_ROARING_STATIC_ASSERT(alignof(art_leaf_t) == alignof(art_node48_t),
                       "Serialization assumes node type alignment is equal");
CFERMAT_ROARING_STATIC_ASSERT(alignof(art_leaf_t) == alignof(art_node256_t),
                       "Serialization assumes node type alignment is equal");
...
size_t art_serialize(const art_t *art, char *buf) {
    ...
    // Alignment for leaves. The rest of the nodes are aligned the same way.
    size_t align_bytes =
        CFERMAT_ROARING_ART_ALIGN_SIZE_RELATIVE(buf, initial_buf, alignof(art_leaf_t));
...
size_t art_frozen_view(const char *buf, size_t maxbytes, art_t *art) {
    ...
    CFERMAT_ROARING_STATIC_ASSERT(sizeof(art->first_free) == sizeof(art->capacities),
    ...
    buf = CFERMAT_ROARING_ART_ALIGN_BUF(buf, alignof(art_leaf_t));
