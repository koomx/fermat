...
#include <fermat/roaring/array_util.h>
...
#define CFERMAT_ROARING_SERIALIZATION_ARRAY_UINT32 1
#define CFERMAT_ROARING_SERIALIZATION_CONTAINER 2
...
static inline bool is_frozen(const roaring_bitmap_t *r) {
    return r->high_low_container.flags & FERMAT_ROARING_FLAG_FROZEN;
}
...
roaring_bitmap_t *roaring_bitmap_of(size_t n_args, ...) {
    ...
    // include long lists
    roaring_bitmap_t *answer = roaring_bitmap_create();
    roaring_bulk_context_t context = CFERMAT_ROARING_ZERO_INITIALIZER;
    va_list ap;
    va_start(ap, n_args);
...
bool roaring_bitmap_internal_validate(const roaring_bitmap_t *r,
    ...
    *reason = NULL;
    ...
    if (ra->flags & ~(FERMAT_ROARING_FLAG_COW | FERMAT_ROARING_FLAG_FROZEN)) {
...
size_t roaring_bitmap_serialize(const roaring_bitmap_t *r, char *buf) {
    ...
    if (portablesize < sizeasarray) {
        buf[0] = CFERMAT_ROARING_SERIALIZATION_CONTAINER;
        return roaring_bitmap_portable_serialize(r, buf + 1) + 1;
    } else {
        buf[0] = CFERMAT_ROARING_SERIALIZATION_ARRAY_UINT32;
...
roaring_bitmap_t *roaring_bitmap_deserialize(const void *buf) {
    ...
    if (bufaschar[0] == CFERMAT_ROARING_SERIALIZATION_ARRAY_UINT32) {
    ...
    } else if (bufaschar[0] == CFERMAT_ROARING_SERIALIZATION_CONTAINER) {
 ...
 */
CFERMAT_ROARING_WARN_UNUSED static bool iter_new_container_partial_init(
 ...
 */
CFERMAT_ROARING_WARN_UNUSED static bool loadfirstvalue(
    roaring_uint32_iterator_t *newit) {
 ...
 * iterator points at, if available.
 */
CFERMAT_ROARING_WARN_UNUSED static bool loadlastvalue(
    roaring_uint32_iterator_t *newit) {
    if (iter_new_container_partial_init(newit)) {
        uint16_t value = 0;
 ...
 */
CFERMAT_ROARING_WARN_UNUSED static bool loadfirstvalue_largeorequal(
...
const roaring_bitmap_t *roaring_bitmap_frozen_view(const char *buf,
    ...
    rb->high_low_container.flags = FERMAT_ROARING_FLAG_FROZEN;
...
}  // extern "C" { namespace fermat::roaring {
#endif
