...
namespace fermat::base_internal {
    ...
    namespace {
...
#endif
            EXPECT_GT(__builtin_infer_alloc_token(sizeof(turbo::container_internal::AlignedType<alignof(void*)>)),
        ...
        TEST(Memory, AlignmentLargerThanBase) {
            ...
            void* mem = turbo::container_internal::Allocate<2>(&alloc, 3);
            EXPECT_EQ(0, reinterpret_cast<uintptr_t>(mem) % 2);
            memcpy(mem, "abc", 3);
            turbo::container_internal::Deallocate<2>(&alloc, mem, 3);
        }
        ...
        TEST(Memory, AllocateDeallocateMatchType) {
            ...
            void* mem = turbo::container_internal::Allocate<1>(&alloc, 1);
            // Verify that it was allocated
            EXPECT_THAT(AllocationMap(), ElementsAre(Pair(_, Gt(0))));
            turbo::container_internal::Deallocate<1>(&alloc, mem, 1);
        ...
        TEST_F(Fixture, ConstructNoArgs) {
            turbo::container_internal::ConstructFromTuple(alloc(), ptr(), std::forward_as_tuple());
            EXPECT_EQ(*ptr(), "");
        }

        TEST_F(Fixture, ConstructOneArg) {
            turbo::container_internal::ConstructFromTuple(alloc(), ptr(), std::forward_as_tuple("abcde"));
            EXPECT_EQ(*ptr(), "abcde");
        }

        TEST_F(Fixture, ConstructTwoArg) {
            turbo::container_internal::ConstructFromTuple(alloc(), ptr(), std::forward_as_tuple(5, 'a'));
            EXPECT_EQ(*ptr(), "aaaaa");
        }

        TEST(PairArgs, NoArgs) {
            EXPECT_THAT(turbo::container_internal::PairArgs(),
        ...
        TEST(PairArgs, TwoArgs) {
            EXPECT_EQ(
                ...
                turbo::container_internal::PairArgs(1, 'A'));
        }

        TEST(PairArgs, Pair) {
            EXPECT_EQ(
                std::make_pair(std::forward_as_tuple(1), std::forward_as_tuple('A')),
                turbo::container_internal::PairArgs(std::make_pair(1, 'A')));
        }

        TEST(PairArgs, Piecewise) {
            EXPECT_EQ(
                std::make_pair(std::forward_as_tuple(1), std::forward_as_tuple('A')),
                turbo::container_internal::PairArgs(std::piecewise_construct, std::forward_as_tuple(1),
                    std::forward_as_tuple('A')));
        }

        TEST(WithConstructed, Simple) {
            EXPECT_EQ(1, turbo::container_internal::WithConstructed<std::string_view>(std::make_tuple(std::string("a")), [](std::string_view str) { return str.size(); }));
        }

        template <class F, class Arg>
        decltype(turbo::container_internal::DecomposeValue(std::declval<F>(), std::declval<Arg>()))
        DecomposeValueImpl(int, F&& f, Arg&& arg) {
            return turbo::container_internal::DecomposeValue(std::forward<F>(f), std::forward<Arg>(arg));
        }
        ...
        decltype(turbo::container_internal::DecomposePair(std::declval<F>(), std::declval<Args>()...))
        DecomposePairImpl(int, F&& f, Args&&... args) {
            return turbo::container_internal::DecomposePair(std::forward<F>(f), std::forward<Args>(args)...);
        }
        ...
        TEST(MapSlotPolicy, ConstKeyAndValue) {
            using slot_policy = turbo::container_internal::map_slot_policy<const CopyableMovableInstance,
        ...
        TEST(MapSlotPolicy, TransferReturnsTrue) {
            {
                using slot_policy = turbo::container_internal::map_slot_policy<int, float>;
                EXPECT_TRUE(
            ...
            }
            {
                ...
                using slot_policy = turbo::container_internal::map_slot_policy<int, NonRelocatable>;
                EXPECT_TRUE(
                    (std::is_same_v<decltype(slot_policy::transfer<std::allocator<char>>(
                                        nullptr, nullptr, nullptr)),
                        std::false_type>));
        ...
        TEST(MapSlotPolicy, DestroyReturnsTrue) {
            ...
            }
            {
                ...
                using slot_policy = turbo::container_internal::map_slot_policy<int, std::unique_ptr<int>>;
                EXPECT_TRUE(
                    (std::is_same_v<decltype(slot_policy::destroy<std::allocator<char>>(
                                        nullptr, nullptr)),
                        std::false_type>));
        ...
        TEST(ApplyTest, TypeErasedApplyToSlotFn) {
            ...
            EXPECT_EQ(
                (turbo::container_internal::TypeErasedApplyToSlotFn<decltype(fn), size_t, /*kIsDefault=*/false>(
                    &fn, &x, seed)),
                (turbo::container_internal::HashElement<decltype(fn), /*kIsDefault=*/false>(fn, seed)(x)));
        }
        ...
        TEST(ApplyTest, TypeErasedDerefAndApplyToSlotFn) {
            ...
            auto fn = [](size_t v) { return v * 2; };
            size_t* x_ptr = &x;
            EXPECT_EQ((turbo::container_internal::TypeErasedDerefAndApplyToSlotFn<decltype(fn), size_t,
        ...
        TEST(HashElement, DefaultHash) {
            ...
            EXPECT_EQ((turbo::container_internal::HashElement<HashWithSeed, /*kIsDefault=*/true>(hash, seed)(x)),
        ...
        TEST(HashElement, NonDefaultHash) {
            ...
            auto fn = [](size_t v) { return v * 2; };
            EXPECT_EQ(
                (turbo::container_internal::HashElement<decltype(fn), /*kIsDefault=*/false>(
                    fn, seed)(x)),
                fn(x) ^ seed);
        }
