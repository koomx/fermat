...
#include <tests/maps/btree_test.h>
...
using ::fermat::test_internal::MovableOnlyInstance;
...
void SetTest() {
  EXPECT_EQ(
      sizeof(fermat::btree_set<K>),
      2 * sizeof(void *) + sizeof(typename fermat::btree_set<K>::size_type));
  using BtreeSet = fermat::btree_set<K>;
...
void MapTest() {
  EXPECT_EQ(
      sizeof(fermat::btree_map<K, K>),
      2 * sizeof(void *) + sizeof(typename fermat::btree_map<K, K>::size_type));
  using BtreeMap = fermat::btree_map<K, K>;
...
void MultiSetTest() {
  EXPECT_EQ(
      sizeof(fermat::btree_multiset<K>),
      2 * sizeof(void *) + sizeof(typename fermat::btree_multiset<K>::size_type));
  using BtreeMSet = fermat::btree_multiset<K>;
  BtreeMultiTest<BtreeMSet, std::multiset<K>>();
}

template <typename K, int N = 256>
void MultiMapTest() {
  EXPECT_EQ(sizeof(fermat::btree_multimap<K, K>),
            2 * sizeof(void *) +
                sizeof(typename fermat::btree_multimap<K, K>::size_type));
  using BtreeMMap = fermat::btree_multimap<K, K>;
...
TEST(Btree, NoHeterogeneousLookupWithoutAlias) {
  using StringSet = fermat::btree_set<std::string, NonTransparentCompare>;
  StringSet s;
  ...
  using StringMultiSet =
      fermat::btree_multiset<std::string, NonTransparentCompare>;
  StringMultiSet ms;
  ms.insert("hello");
  ms.insert("world");
  ms.insert("world");
...
TEST(Btree, HeterogeneousLookupDoesntDegradePerformance) {
  using StringSet = fermat::btree_set<StringLike>;
  StringSet s;
  for (int i = 0; i < 100; ++i) {
...
TEST(Btree, SwapKeyCompare) {
  using SubstringSet = fermat::btree_set<std::string, SubstringLess>;
...
TEST(Btree, Comparison) {
  ...
  fermat::btree_set<int64_t> my_set;
  for (int i = 0; i < kSetSize; ++i) {
    my_set.insert(i);
  }
  fermat::btree_set<int64_t> my_set_copy(my_set);
  ...
  fermat::btree_map<std::string, int64_t> my_map;
  for (int i = 0; i < kSetSize; ++i) {
    my_map[std::string(i, 'a')] = i;
  }
  fermat::btree_map<std::string, int64_t> my_map_copy(my_map);
...
TEST(Btree, RangeCtorSanity) {
  ...
  fermat::btree_multiset<int> tmset(ivec.begin(), ivec.end());
  fermat::btree_multimap<int, int> tmmap(imap.begin(), imap.end());
  fermat::btree_set<int> tset(ivec.begin(), ivec.end());
  fermat::btree_map<int, int> tmap(imap.begin(), imap.end());
...
class BtreeMapTest : public ::testing::Test {
 public:
  ...
  static bool IsLinear() {
    return BtreeNodePeer::UsesLinearNodeSearch<fermat::btree_map<K, int, C>>();
  }
};
...
TEST(Btree, BtreeMapCanHoldMoveOnlyTypes) {
  fermat::btree_map<std::string, std::unique_ptr<std::string>> m;
...
TEST(Btree, InitializerListConstructor) {
  fermat::btree_set<std::string> set({"a", "b"});
  EXPECT_EQ(set.count("a"), 1);
  EXPECT_EQ(set.count("b"), 1);

  fermat::btree_multiset<int> mset({1, 1, 4});
  EXPECT_EQ(mset.count(1), 2);
  EXPECT_EQ(mset.count(4), 1);

  fermat::btree_map<int, int> map({{1, 5}, {2, 10}});
  EXPECT_EQ(map[1], 5);
  EXPECT_EQ(map[2], 10);

  fermat::btree_multimap<int, int> mmap({{1, 5}, {1, 10}});
...
TEST(Btree, InitializerListInsert) {
  fermat::btree_set<std::string> set;
  set.insert({"a", "b"});
  EXPECT_EQ(set.count("a"), 1);
  EXPECT_EQ(set.count("b"), 1);

  fermat::btree_multiset<int> mset;
  mset.insert({1, 1, 4});
  EXPECT_EQ(mset.count(1), 2);
  EXPECT_EQ(mset.count(4), 1);

  fermat::btree_map<int, int> map;
  map.insert({{1, 5}, {2, 10}});
  ...
  fermat::btree_multimap<int, int> mmap;
...
TEST(Btree, RValueInsert) {
  ...
  fermat::btree_set<MovableOnlyInstance> set;
  ...
  fermat::btree_multiset<MovableOnlyInstance> mset;
  ...
  fermat::btree_map<int, MovableOnlyInstance> map;
  ...
  fermat::btree_multimap<int, MovableOnlyInstance> mmap;
...
TEST(Btree, MovesComparisonsCopiesSwapsTracking) {
  ...
  if (sizeof(void *) == 8) {
    EXPECT_EQ(BtreeNodePeer::GetNumSlotsPerNode<fermat::btree_set<int32_t>>(),
...
TEST(Btree, BtreeMapCanHoldNoDefaultCtorTypes) {
  fermat::btree_map<NoDefaultCtor, NoDefaultCtor> m;
...
TEST(Btree, BtreeMultimapCanHoldNoDefaultCtorTypes) {
  fermat::btree_multimap<NoDefaultCtor, NoDefaultCtor> m;
...
TEST(Btree, BtreeMultimapEmplace) {
  ...
  const char value0[] = "a";
  fermat::btree_multimap<int, std::string> m;
...
TEST(Btree, StrSplitCompatible) {
  const fermat::btree_set<std::string> split_set = turbo::str_split("a,b,c", ',');
  const fermat::btree_set<std::string> expected_set = {"a", "b", "c"};

  EXPECT_EQ(split_set, expected_set);
}

TEST(Btree, KeyComp) {
  fermat::btree_set<int> s;
  EXPECT_TRUE(s.key_comp()(1, 2));
  EXPECT_FALSE(s.key_comp()(2, 2));
  EXPECT_FALSE(s.key_comp()(2, 1));

  fermat::btree_map<int, int> m1;
  ...
  fermat::btree_map<std::string, int> m2;
...
TEST(Btree, MapValueCompProtected) {
  ...
  using value_compare = fermat::btree_map<int, int, key_compare>::value_compare;
...
TEST(Btree, DefaultConstruction) {
  fermat::btree_set<int> s;
  fermat::btree_map<int, int> m;
  fermat::btree_multiset<int> ms;
  fermat::btree_multimap<int, int> mm;
...
TEST(Btree, SwissTableHashable) {
  ...
  using set = fermat::btree_set<int>;
  ...
  using mset = fermat::btree_multiset<int>;
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly({
      mset{},
      mset{1},
      mset{1, 1},
  ...
  using map = fermat::btree_map<int, int>;
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly({
      map{},
      map{{1, 0}},
      map{{1, 1}},
  ...
  using mmap = fermat::btree_multimap<int, int>;
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly({
      mmap{},
      mmap{{1, 0}},
      mmap{{1, 1}},
...
TEST(Btree, ComparableSet) {
  fermat::btree_set<int> s1 = {1, 2};
  fermat::btree_set<int> s2 = {2, 3};
  EXPECT_LT(s1, s2);
  EXPECT_LE(s1, s2);
  EXPECT_LE(s1, s1);
  EXPECT_GT(s2, s1);
...
TEST(Btree, ComparableMultiset) {
  fermat::btree_multiset<int> s1 = {1, 2};
  fermat::btree_multiset<int> s2 = {2, 3};
  EXPECT_LT(s1, s2);
  EXPECT_LE(s1, s2);
  EXPECT_LE(s1, s1);
  EXPECT_GT(s2, s1);
...
TEST(Btree, ComparableMap) {
  fermat::btree_map<int, int> s1 = {{1, 2}};
  fermat::btree_map<int, int> s2 = {{2, 3}};
  EXPECT_LT(s1, s2);
  EXPECT_LE(s1, s2);
  EXPECT_LE(s1, s1);
  EXPECT_GT(s2, s1);
...
TEST(Btree, ComparableMultimap) {
  fermat::btree_multimap<int, int> s1 = {{1, 2}};
  fermat::btree_multimap<int, int> s2 = {{2, 3}};
  EXPECT_LT(s1, s2);
  EXPECT_LE(s1, s2);
  EXPECT_LE(s1, s1);
  EXPECT_GT(s2, s1);
...
TEST(Btree, ComparableSetWithCustomComparator) {
  ...
  fermat::btree_set<int, std::greater<int>> s1 = {1, 2};
  fermat::btree_set<int, std::greater<int>> s2 = {2, 3};
  EXPECT_LT(s1, s2);
  EXPECT_LE(s1, s2);
  EXPECT_LE(s1, s1);
  EXPECT_GT(s2, s1);
...
TEST(Btree, EraseReturnsIterator) {
  fermat::btree_set<int> set = {1, 2, 3, 4, 5};
...
TEST(Btree, ExtractTracking) {
  TestExtractWithTrackingForSet<fermat::btree_set<MovableOnlyInstance>>();
  TestExtractWithTrackingForSet<fermat::btree_multiset<MovableOnlyInstance>>();
  TestExtractWithTrackingForMap<
      fermat::btree_map<CopyableMovableInstance, MovableOnlyInstance>>();
  TestExtractWithTrackingForMap<
      fermat::btree_multimap<CopyableMovableInstance, MovableOnlyInstance>>();
}

TEST(Btree, ExtractAndInsertNodeHandleMultiSet) {
  fermat::btree_multiset<int> src1 = {1, 2, 3, 3, 4, 5};
  auto nh = src1.extract(src1.find(3));
  EXPECT_THAT(src1, ElementsAre(1, 2, 3, 4, 5));
  fermat::btree_multiset<int> other;
  auto res = other.insert(std::move(nh));
  EXPECT_THAT(other, ElementsAre(3));
  EXPECT_EQ(res, other.find(3));

  fermat::btree_multiset<int> src2 = {3, 4};
  nh = src2.extract(src2.find(3));
  EXPECT_THAT(src2, ElementsAre(4));
  res = other.insert(std::move(nh));
...
TEST(Btree, ExtractAndInsertNodeHandleMap) {
  fermat::btree_map<int, int> src1 = {{1, 2}, {3, 4}, {5, 6}};
  auto nh = src1.extract(src1.find(3));
  EXPECT_THAT(src1, ElementsAre(Pair(1, 2), Pair(5, 6)));
  fermat::btree_map<int, int> other;
  fermat::btree_map<int, int>::insert_return_type res =
  ...
  fermat::btree_map<int, int> src2 = {{3, 6}};
  nh = src2.extract(src2.find(3));
...
TEST(Btree, ExtractAndInsertNodeHandleMultiMap) {
  fermat::btree_multimap<int, int> src1 = {{1, 2}, {3, 4}, {5, 6}};
  auto nh = src1.extract(src1.find(3));
  EXPECT_THAT(src1, ElementsAre(Pair(1, 2), Pair(5, 6)));
  fermat::btree_multimap<int, int> other;
  auto res = other.insert(std::move(nh));
  EXPECT_THAT(other, ElementsAre(Pair(3, 4)));
  EXPECT_EQ(res, other.find(3));

  fermat::btree_multimap<int, int> src2 = {{3, 6}};
  nh = src2.extract(src2.find(3));
  EXPECT_TRUE(src2.empty());
  res = other.insert(std::move(nh));
...
TEST(Btree, ExtractMultiMapEquivalentKeys) {
  ...
  fermat::btree_multimap<std::string, int> map;
  for (int i = 0; i < 100; ++i) {
    for (int j = 0; j < 100; ++j) {
...
TEST(Btree, ExtractAndGetNextSet) {
  fermat::btree_set<int> src = {1, 2, 3, 4, 5};
  auto it = src.find(3);
...
TEST(Btree, ExtractAndGetNextMultiSet) {
  fermat::btree_multiset<int> src = {1, 2, 3, 4, 5};
  auto it = src.find(3);
  auto extracted_and_next = src.extract_and_get_next(it);
  EXPECT_THAT(src, ElementsAre(1, 2, 4, 5));
  EXPECT_EQ(extracted_and_next.node.value(), 3);
...
TEST(Btree, ExtractAndGetNextMap) {
  fermat::btree_map<int, int> src = {{1, 2}, {3, 4}, {5, 6}};
  auto it = src.find(3);
  auto extracted_and_next = src.extract_and_get_next(it);
...
TEST(Btree, ExtractAndGetNextMultiMap) {
  fermat::btree_multimap<int, int> src = {{1, 2}, {3, 4}, {5, 6}};
  auto it = src.find(3);
  auto extracted_and_next = src.extract_and_get_next(it);
  EXPECT_THAT(src, ElementsAre(Pair(1, 2), Pair(5, 6)));
  EXPECT_EQ(extracted_and_next.node.key(), 3);
...
TEST(Btree, ExtractDoesntCauseExtraMoves) {
  ...
  using Set = fermat::btree_set<MovableOnlyInstance>;
...
TEST(Btree, InsertHintNodeHandle) {
  ...
  // Test that insert works correctly when the hint is right or wrong.
  {
    ...
    EXPECT_THAT(src, ElementsAre(1, 2, 4, 5));
    fermat::btree_set<int> other = {0, 100};
    // Test a correct hint.
  ...
  fermat::btree_multiset<InsertMultiHintData, InsertMultiHintDataKeyCompare> src =
      {{1, 2}, {3, 4}, {3, 5}};
  auto nh = src.extract(src.lower_bound(3));
  EXPECT_EQ(nh.value(), (InsertMultiHintData{3, 4}));
  fermat::btree_multiset<InsertMultiHintData, InsertMultiHintDataKeyCompare>
      other = {{3, 1}, {3, 2}, {3, 3}};
...
TEST(Btree, MergeIntoUniqueContainers) {
  fermat::btree_set<int, IntCompareToCmp> src1 = {1, 2, 3};
  fermat::btree_multiset<int> src2 = {3, 4, 4, 5};
  fermat::btree_set<int> dst;
...
TEST(Btree, MergeIntoUniqueContainersWithCompareTo) {
  fermat::btree_set<int, IntCompareToCmp> src1 = {1, 2, 3};
  fermat::btree_multiset<int> src2 = {3, 4, 4, 5};
  fermat::btree_set<int, IntCompareToCmp> dst;
...
TEST(Btree, MergeIntoMultiContainersWithCompareTo) {
  fermat::btree_set<int, IntCompareToCmp> src1 = {1, 2, 3};
  fermat::btree_multiset<int> src2 = {3, 4, 4, 5};
  fermat::btree_multiset<int, IntCompareToCmp> dst;
...
TEST(Btree, MergeIntoMultiMapsWithDifferentComparators) {
  fermat::btree_map<int, int, IntCompareToCmp> src1 = {{1, 1}, {2, 2}, {3, 3}};
  fermat::btree_multimap<int, int, std::greater<int>> src2 = {
      {5, 5}, {4, 1}, {4, 4}, {3, 2}};
  fermat::btree_multimap<int, int> dst;
...
TEST(Btree, MergeIntoSetMovableOnly) {
  fermat::btree_set<MovableOnlyInstance> src;
  src.insert(MovableOnlyInstance(1));
  fermat::btree_multiset<MovableOnlyInstance> dst1;
  dst1.insert(MovableOnlyInstance(2));
  fermat::btree_set<MovableOnlyInstance> dst2;
...
TEST(Btree, TryEmplaceWithHintWorks) {
  ...
  fermat::btree_map<int, int, CheckedCompareOptedOutCmp<Cmp>> m(cmp);
  for (int i = 0; i < 128; ++i) {
    m.emplace(i, i);
  }
...
TEST(Btree, TryEmplaceWithBadHint) {
  fermat::btree_map<int, int> m = {{1, 1}, {9, 9}};
...
using BtreeSetAlloc = fermat::btree_set<int, std::less<int>, Alloc>;

TEST(Btree, AllocatorPropagation) {
  fermat::container_internal::TestAllocPropagation<BtreeSetAlloc>();
}

TEST(Btree, MinimumAlignmentAllocator) {
  fermat::btree_set<int8_t, std::less<int8_t>, fermat::container_internal::MinimumAlignmentAlloc<int8_t>> set;
...
TEST(Btree, EraseIf) {
  ...
  {
    fermat::btree_set<int> s = {1, 3, 5, 6, 100};
    EXPECT_EQ(erase_if(s, [](int k) { return k > 3; }), 3);
    EXPECT_THAT(s, ElementsAre(1, 3));
  }
  {
    fermat::btree_multiset<int> s = {1, 3, 3, 5, 6, 6, 100};
    EXPECT_EQ(erase_if(s, [](int k) { return k <= 3; }), 3);
    EXPECT_THAT(s, ElementsAre(5, 6, 6, 100));
  }
  {
    fermat::btree_map<int, int> m = {{1, 1}, {3, 3}, {6, 6}, {100, 100}};
    EXPECT_EQ(
  ...
  {
    fermat::btree_multimap<int, int> m = {{1, 1}, {3, 3}, {3, 6},
                                        {6, 6}, {6, 7}, {100, 6}};
    EXPECT_EQ(
        erase_if(m,
...
TEST(Btree, InsertOrAssign) {
  fermat::btree_map<int, int> m = {{1, 1}, {3, 3}};
...
TEST(Btree, InsertOrAssignMovableOnly) {
  fermat::btree_map<int, MovableOnlyInstance> m;
  using value_type = typename decltype(m)::value_type;
...
TEST(Btree, SetRangeConstructorAndInsertSupportExplicitConversionComparable) {
  ...
  fermat::btree_set<std::string> name_set1{std::begin(names), std::end(names)};
  EXPECT_THAT(name_set1, ElementsAreArray(names));

  fermat::btree_set<std::string> name_set2;
...
TEST(Btree,
  ...
  fermat::btree_set<ConstructorCounted, ConstructorCountedCompare> set{
...
TEST(Btree,
  ...
  fermat::btree_set<std::vector<void *>> s1{std::begin(i), std::end(i)};
  EXPECT_THAT(s1, ElementsAre(IsEmpty(), ElementsAre(IsNull())));

  fermat::btree_set<std::vector<void *>> s2;
...
TEST(Btree, MapRangeConstructorAndInsertSupportExplicitConversionComparable) {
  ...
  fermat::btree_map<std::string, int> name_map1{std::begin(names),
                                              std::end(names)};
  EXPECT_THAT(name_map1, ElementsAre(Pair("n1", 1), Pair("n2", 2)));

  fermat::btree_map<std::string, int> name_map2;
...
TEST(Btree,
  ...
  fermat::btree_map<ConstructorCounted, int, ConstructorCountedCompare> map{
      std::begin(i), std::end(i)};
...
TEST(Btree,
  ...
  fermat::btree_map<std::vector<void *>, int> m1{std::begin(i), std::end(i)};
  EXPECT_THAT(m1,
              ElementsAre(Pair(IsEmpty(), 1), Pair(ElementsAre(IsNull()), 2)));

  fermat::btree_map<std::vector<void *>, int> m2;
...
TEST(Btree, HeterogeneousTryEmplace) {
  fermat::btree_map<std::string, int> m;
  std::string s = "key";
...
TEST(Btree, NodeHandleMutableKeyAccess) {
  {
    fermat::btree_map<std::string, std::string> map;
  ...
  // Also for multimap.
  {
    fermat::btree_multimap<std::string, std::string> map;
...
TYPED_TEST(BtreeMultiKeyTest, EqualRange) {
  fermat::btree_set<MultiKey, TypeParam> set;
  for (int i = 0; i < 100; ++i) {
    for (int j = 0; j < 100; ++j) {
      set.insert({i, j});
    }
...
TYPED_TEST(BtreeMultiKeyTest, Count) {
  const fermat::btree_set<MultiKey, TypeParam> set = {
      {1, 1}, {2, 1}, {2, 2}, {3, 1}};
...
TEST(Btree, AllocConstructor) {
  using Alloc = fermat::container_internal::CountingAllocator<int>;
  using Set = fermat::btree_set<int, std::less<int>, Alloc>;
...
TEST(Btree, SupportsFunctionPtrComparator) {
  fermat::btree_set<int, decltype(IntCmp) *> set(IntCmp);
  set.insert({1, 2, 3});
  EXPECT_THAT(set, ElementsAre(1, 2, 3));
  EXPECT_TRUE(set.key_comp()(1, 2));
  EXPECT_TRUE(set.value_comp()(1, 2));

  fermat::btree_map<int, int, decltype(IntCmp) *> map(&IntCmp);
  map[1] = 1;
...
TEST(Btree,
  ...
  fermat::btree_set<MultiKey, TransparentPassThroughComp<MultiKeyComp>> set;
  set.insert(MultiKey{1, 2});
  EXPECT_TRUE(set.contains(1));
}

TEST(Btree, ConstructImplicitlyWithUnadaptedComparator) {
  fermat::btree_set<MultiKey, MultiKeyComp> set = {{}, MultiKeyComp{}};
  EXPECT_TRUE(set.empty());
}

TEST(Btree, InvalidComparatorsCaught) {
  if (!fermat::container_internal::IsAssertEnabled()) GTEST_SKIP() << "Assertions not enabled.";

  {
    ...
    fermat::btree_set<int, ZeroAlwaysLessCmp> set;
  ...
  }
  {
    ...
    fermat::btree_set<int, ThreeWayAlwaysLessCmp> set;
    EXPECT_DEATH(set.insert({0, 1, 2}), "is_self_equivalent");
  }
  {
    ...
    fermat::btree_set<int, SumGreaterZeroCmp> set;
  ...
  }
  {
    ...
    fermat::btree_set<int, ThreeWaySumGreaterZeroCmp> set;
  ...
  ClockTime c = {6, 0};
  {
    ...
    fermat::btree_set<ClockTime, NonTransitiveTimeCmp> set;
    EXPECT_DEATH(set.insert({a, b, c}), "is_ordered_correctly");
    fermat::btree_multiset<ClockTime, NonTransitiveTimeCmp> mset;
  ...
  }
  {
    ...
    fermat::btree_set<ClockTime, ThreeWayNonTransitiveTimeCmp> set;
    EXPECT_DEATH(set.insert({a, b, c}), "is_ordered_correctly");
    fermat::btree_multiset<ClockTime, ThreeWayNonTransitiveTimeCmp> mset;
    EXPECT_DEATH(mset.insert({a, a, b, b, c, c}), "is_ordered_correctly");
  }
}
...
TEST(Btree, MutatedKeysCaught) {
  ...
  {
    fermat::btree_set<int *, IntPtrCmp> set;
    int arr[] = {0, 1, 2};
  ...
  {
    fermat::btree_multiset<int *, IntPtrCmp> set;
    int arr[] = {0, 1, 2};
...
TEST(Btree, OnlyConstructibleByAllocatorType) {
  ...
  {
    fermat::btree_set<OnlyConstructibleByAllocator,
  ...
  {
    fermat::btree_multiset<OnlyConstructibleByAllocator,
                         OnlyConstructibleByAllocatorComp,
                         OnlyConstructibleAllocator<>>
        set;
    set.emplace(1);
  ...
  {
    fermat::btree_map<OnlyConstructibleByAllocator, int,
                    OnlyConstructibleByAllocatorComp,
                    OnlyConstructibleAllocator<>>
        map;
    map.emplace(1);
  ...
  {
    fermat::btree_multimap<OnlyConstructibleByAllocator, int,
                         OnlyConstructibleByAllocatorComp,
                         OnlyConstructibleAllocator<>>
        map;
    map.emplace(1);
...
TEST(Btree, NotAssignableType) {
  {
    fermat::btree_set<NotAssignable> set;
    set.emplace(1);
    set.emplace_hint(set.end(), 2);
  ...
  {
    fermat::btree_multiset<NotAssignable> set;
    set.emplace(1);
    set.emplace_hint(set.end(), 2);
  ...
  {
    fermat::btree_map<NotAssignable, int> map;
  ...
  {
    fermat::btree_multimap<NotAssignable, int> map;
    map.emplace(NotAssignable(1), 1);
    map.emplace_hint(map.end(), NotAssignable(2), 2);
...
TEST(Btree, ReusePoisonMemory) {
  ...
  using Set = fermat::btree_set<int64_t, std::less<int64_t>, Alloc>;
  ArenaLike arena;
  Alloc alloc(&arena);
  Set set(alloc);
...
TEST(Btree, IteratorDifference) {
  ...
  turbo::c_shuffle(vec, rng);

  fermat::btree_set<int> set;
  for (int i : vec) set.insert(i);

  for (int i = 0; i < 1000; ++i) {
    size_t begin = std::uniform_int_distribution<size_t>(0, set.size() - 1)(rng);
    size_t end = std::uniform_int_distribution<size_t>(begin, set.size() - 1)(rng);
...
TEST(Btree, IteratorAddition) {
  ...
  for (int i = 0; i < 1000; ++i) {
    int begin = std::uniform_int_distribution<int>(0, kSetSize - 1)(rng);
    int end = std::uniform_int_distribution<int>(begin, kSetSize - 1)(rng);
    ASSERT_LE(begin, end);
...
TEST(Btree, IteratorAdditionOutOfBounds) {
  const fermat::btree_set<int> set({5});
  ...
  if (fermat::container_internal::IsAssertEnabled()) {
...
TEST(Btree, InvalidIteratorComparison) {
  ...
  fermat::btree_set<int> set1, set2;
  for (int i = 0; i < 1000; ++i) {
    set1.insert(i);
    set2.insert(i);
  }

  typename fermat::btree_set<int>::iterator iter1, iter2;
...
TEST(Btree, FieldTypeEqualsSlotType) {
  ...
  using set_type = fermat::btree_set<uint8_t>;
