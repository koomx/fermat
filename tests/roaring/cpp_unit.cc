...
#include <fermat/roaring/roaring.h>  // access to pure C exported API for testing
#include <fermat/roaring/roaring.hpp>
#include <fermat/roaring/roaring64map.hpp>

#include "config.h"

using fermat::roaring::Roaring;       // the C++ wrapper class
using fermat::roaring::Roaring64Map;  // C++ class extended for 64-bit numbers

#include "roaring64map_checked.hh"
#define FERMAT_ROARING_TEST_SUITE CppUnit
#include "test.h"
...
DEFINE_TEST(test_cpp_add_bulk) {
    ...
    Roaring r1;
    Roaring r2;
    fermat::roaring::BulkContext bulk_context;
...
#if FERMAT_ROARING_UNSAFE_FROZEN_TESTS
// This test is unsafe, as it may trigger unaligned memory access
// It is only enabled if FERMAT_ROARING_UNSAFE_FROZEN_TESTS is defined.
...
#endif  // FERMAT_ROARING_UNSAFE_FROZEN_TESTS
...
#else   // FERMAT_ROARING_EXCEPTIONS
    roaring = Roaring64Map::readSafe(buf1.data(), buf1.size());
#endif  // FERMAT_ROARING_EXCEPTIONS
