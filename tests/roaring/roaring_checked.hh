...
#ifndef INCLUDE_FERMAT_ROARING_CHECKED_HH_
#define INCLUDE_FERMAT_ROARING_CHECKED_HH_
...
#include <fermat/roaring/roaring.hpp>            // contains Roaring unchecked class

namespace doublechecked {  // put the checked class in its own namespace

class Roaring {
   public:                     // members public to allow tests access to them
    fermat::roaring::Roaring plain;    // ordinary Roaring bitset wrapper class
   ...
   public:
    ...
    Roaring(fermat::roaring::api::roaring_bitmap_t *s) noexcept : plain(s) {
    ...
    //
    Roaring(fermat::roaring::Roaring &&other_plain) {
    ...
    void iterate(fermat::roaring::api::roaring_iterator iterator, void *ptr) const {
    ...
    static Roaring read(const char *buf, bool portable = true) {
        auto plain = fermat::roaring::Roaring::read(buf, portable);
        return Roaring(std::move(plain));
    }

    static Roaring readSafe(const char *buf, size_t maxbytes) {
        auto plain = fermat::roaring::Roaring::readSafe(buf, maxbytes);
        return Roaring(std::move(plain));
    }
    ...
    static Roaring fastunion(size_t n, const Roaring **inputs) {
        auto plain_inputs = new const fermat::roaring::Roaring *[n];
        for (size_t i = 0; i < n; ++i) plain_inputs[i] = &inputs[i]->plain;
        Roaring ans(fermat::roaring::Roaring::fastunion(n, plain_inputs));
        delete[] plain_inputs;
    ...
    typedef fermat::roaring::RoaringSetBitBiDirectionalIterator const_iterator;

    const_iterator begin() const {
        return fermat::roaring::RoaringSetBitBiDirectionalIterator(plain);
    }

    const_iterator &end() const {
        static fermat::roaring::RoaringSetBitBiDirectionalIterator e(plain, true);
        return e;
    }
};
...
#endif  // INCLUDE_FERMAT_ROARING_CHECKED_HH_
