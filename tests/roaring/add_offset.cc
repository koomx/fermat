...
#include <fermat/roaring/misc/configreport.h>
#include <fermat/roaring/roaring.h>

// include internal headers for invasive testing
#include <fermat/roaring/containers/containers.h>
#include <fermat/roaring/roaring_array.h>

#ifdef __cplusplus  // stronger type checking errors if C built in C++ mode
using namespace fermat::roaring::internal;
#endif

#define FERMAT_ROARING_TEST_SUITE AddOffset
#include "test.h"
...
#define FERMAT_ROARING_ADD_OFFSET_TEST_CASE(vals, offset) \
    {                                              \
        {                                          \
...
#define FERMAT_ROARING_ADD_OFFSET_TEST(state)                                       \
    {                                                                        \
...
TEST(AddOffset, All) {
    ...
    uint32_t dense_bitmap[100000];
    size_t i, j;
    for (i = 0; i < ARRAY_SIZE(dense_bitmap); i++) {
        dense_bitmap[i] = i * 1000;
    }
    assert_true(i == ARRAY_SIZE(dense_bitmap));

    roaring_add_offset_test_state_t roaring_state[64] = {
        FERMAT_ROARING_ADD_OFFSET_TEST_CASE(sparse_bitmap, 0),
        FERMAT_ROARING_ADD_OFFSET_TEST_CASE(sparse_bitmap, 10000),
        FERMAT_ROARING_ADD_OFFSET_TEST_CASE(sparse_bitmap, 25000),
        FERMAT_ROARING_ADD_OFFSET_TEST_CASE(sparse_bitmap, 83097),
        FERMAT_ROARING_ADD_OFFSET_TEST_CASE(sparse_bitmap, UINT32_MAX),
        FERMAT_ROARING_ADD_OFFSET_TEST_CASE(sparse_bitmap, UINT32_MAX - UINT16_MAX),
        FERMAT_ROARING_ADD_OFFSET_TEST_CASE(sparse_bitmap,
                                     UINT32_MAX - UINT16_MAX + 1),
        FERMAT_ROARING_ADD_OFFSET_TEST_CASE(sparse_bitmap, ((uint64_t)1) << 50),
        FERMAT_ROARING_ADD_OFFSET_TEST_CASE(sparse_bitmap, 281474976710657L),
    };
    i = 9;
    for (int64_t offset = 3; offset < 1000000; offset *= 3) {
        roaring_add_offset_test_state_t state =
            FERMAT_ROARING_ADD_OFFSET_TEST_CASE(dense_bitmap, offset);
    ...
    for (i = 0; i < ARRAY_SIZE(container_state); i++) {
        void *st = &container_state[i];
        ASSERT_EQ(setup_container_add_offset_test(&st), 0);
        container_add_offset_test(&st);
        teardown_container_add_offset_test(&st);
    }
    const size_t n_roaring = i;
    for (j = 0; j < n_roaring; j++) {
        void *st = &roaring_state[j];
        ASSERT_EQ(setup_roaring_add_offset_test(&st), 0);
        roaring_add_offset_test(&st);
        teardown_roaring_add_offset_test(&st);
    }
}
