...
#include "config.h"
#define FERMAT_ROARING_TEST_SUITE RobustDeserialization
#include "test.h"
...
static void invalid_deserialize_test(const void* data, size_t size,
    ...
    size_t serialized_size =
        roaring_bitmap_portable_deserialize_size((const char*)data, size);
    ...
    assert_int_equal(roaring_bitmap_portable_deserialize_size((const char*)data, size - 1),
                     0);
    roaring_bitmap_t* bitmap =
        roaring_bitmap_portable_deserialize_safe((const char*)data, size);
    if (bitmap != NULL) {
    ...
    bitmap = roaring_bitmap_portable_deserialize_safe((const char*)data, size - 1);
    assert_null(bitmap);
}
