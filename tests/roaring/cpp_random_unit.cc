...
#define FERMAT_ROARING_TEST_SUITE CppRandom
#include "test.h"
...
// option FERMAT_ROARING_SANITIZE).
//
...
#ifdef FERMAT_ROARING_CPP_RANDOM_PRINT_STATUS
        printf("[%lu]: %lu %lu %lu\n", step,
...
DEFINE_TEST(safe_test_lazari) {
    unsigned char payload[] = {0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                               ...
                               0x00, 0x6a, 0x6a, 0xd4};
    try {
        fermat::roaring::Roaring64Map::readSafe((const char *)payload, sizeof(payload));
