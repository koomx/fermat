...
#define FERMAT_ROARING_TEST_SUITE Toplevel
#include "test.h"
...
DEFINE_TEST(issue433) {
    ...
    char *data = (char*)roaring_malloc(len);
...
void read_backward_compare(roaring_bitmap_t *r, const uint32_t *ref_values,
    ...
    uint32_t *buffer =
        (uint32_t*)malloc(sizeof(uint32_t) * (step == UINT32_MAX ? 65536 : step));
...
DEFINE_TEST(convert_to_bitset) {
    ...
    fermat::roaring::bitset_t *bitset = fermat::roaring::bitset_create();
