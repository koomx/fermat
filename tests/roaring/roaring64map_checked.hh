...
//
//     #ifdef FERMAT_ROARING_DOUBLECHECK_CPP
...
#ifndef INCLUDE_FERMAT_ROARING_64_MAP_CHECKED_HH_
#define INCLUDE_FERMAT_ROARING_64_MAP_CHECKED_HH_
...
#define FERMAT_ROARING_CPP_NAMESPACE unchecked  // can't be overridden if global
#include "fermat/roaring/roaring64map.hpp"  // contains Roaring64Map unchecked class

namespace doublechecked {  // put the checked class in its own namespace

class Roaring64Map {
   public:  // members public to allow tests access to them
    fermat::roaring::Roaring64Map plain;  // ordinary Roaring64Map bitset wrapper class
   ...
   public:
    ...
    //
    Roaring64Map(fermat::roaring::Roaring64Map &&other_plain) {
    ...
    void iterate(fermat::roaring::api::roaring_iterator64 iterator, void *ptr) const {
    ...
    static Roaring64Map read(const char *buf, bool portable = true) {
        auto plain = fermat::roaring::Roaring64Map::read(buf, portable);
        return Roaring64Map(std::move(plain));
    }

    static Roaring64Map readSafe(const char *buf, size_t maxbytes) {
        auto plain = fermat::roaring::Roaring64Map::readSafe(buf, maxbytes);
        return Roaring64Map(std::move(plain));
    }
    ...
    static Roaring64Map fastunion(size_t n, const Roaring64Map **inputs) {
        auto plain_inputs = new const fermat::roaring::Roaring64Map *[n];
        for (size_t i = 0; i < n; ++i) plain_inputs[i] = &inputs[i]->plain;
        Roaring64Map ans(fermat::roaring::Roaring64Map::fastunion(n, plain_inputs));
        delete[] plain_inputs;
    ...
    typedef fermat::roaring::Roaring64MapSetBitForwardIterator const_iterator;

    const_iterator begin() const {
        return fermat::roaring::Roaring64MapSetBitForwardIterator(plain);
    }

    const_iterator &end() const {
        static fermat::roaring::Roaring64MapSetBitForwardIterator e(plain, true);
        return e;
    }
};
...
#endif  // INCLUDE_FERMAT_ROARING_64_MAP_CHECKED_HH_
