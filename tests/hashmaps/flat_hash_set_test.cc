...
#include <tests/hashmaps/internal/unordered_set_lookup_test.h>
#include <tests/hashmaps/internal/unordered_set_members_test.h>
#include <tests/hashmaps/internal/unordered_set_modifiers_test.h>
...
// Check that fermat::flat_hash_set works in a global constructor.
struct BeforeMain {
  BeforeMain() {
    fermat::flat_hash_set<int> x;
    x.insert(1);
...
template <class T>
using Set =
    fermat::flat_hash_set<T, StatefulTestingHash, StatefulTestingEqual, Alloc<T>>;
...
TEST(FlatHashSet, EmplaceString) {
  ...
  fermat::flat_hash_set<std::string_view> hs(v.begin(), v.end());
...
TEST(FlatHashSet, MergeExtractInsert) {
  ...
  fermat::flat_hash_set<std::unique_ptr<int>, Hash, Eq> set1, set2;
...
TEST(FlatHashSet, IsDefaultHash) {
  using fermat::container_internal::hashtable_debug_internal::
...
TEST(FlatHashSet, FromRange) {
  ...
  fermat::flat_hash_set<int> s(std::from_range, v);
  EXPECT_THAT(s, UnorderedElementsAre(1, 2, 3, 4, 5));
}
