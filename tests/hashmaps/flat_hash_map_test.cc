...
// Check that fermat::flat_hash_map works in a global constructor.
struct BeforeMain {
  BeforeMain() {
    fermat::flat_hash_map<int, int> x;
    x.insert({1, 1});
...
TEST(FlatHashMap, Relocatability) {
  ...
  static_assert(
      std::is_same_v<
          decltype(fermat::container_internal::FlatHashMapPolicy<
  ...
  EXPECT_TRUE(
      (std::is_same_v<decltype(fermat::container_internal::FlatHashMapPolicy<
                               int, NonRelocatable>::
...
TEST(FlatHashMap, IteratesMsan) {
  ...
  std::vector<fermat::flat_hash_map<int, balast>> garbage;
  for (int i = 0; i < 100; ++i) {
    fermat::flat_hash_map<int, balast> t;
    for (int j = 0; j < 100; ++j) {
      t[j];
...
TEST(FlatHashMap, MergeExtractInsert) {
  ...
  fermat::flat_hash_map<int, int> m = {{1, 7}, {2, 9}};
...
TEST(Iterator, InconsistentHashEqFunctorsValidation) {
  ...
  fermat::flat_hash_map<InconsistentHashEqType, int> m;
...
TEST(FlatHashMap, FromRange) {
  ...
  fermat::flat_hash_map<int, int> m(std::from_range, v);
