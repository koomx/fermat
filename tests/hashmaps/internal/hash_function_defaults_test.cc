...
#include <string_view>
#include <tests/base/cord_test_helpers.h>
...
namespace fermat {
            ...
            TEST(HashCord, RandomCord) {
                ...
                for (int i = 0; i < 1000; ++i) {
                    const int number_of_segments =
                        std::uniform_int_distribution<int>(0, 9)(bitgen);
                    std::vector<std::string> pieces;
                    for (size_t s = 0; s < number_of_segments; ++s) {
                        std::string str;
                        str.resize(std::uniform_int_distribution<int>(0, 4095)(bitgen));
                        // MSVC needed the explicit return type in the lambda.
                        std::generate(str.begin(), str.end(), [&]() -> char {
                            return static_cast<char>(
                                std::uniform_int_distribution<int>(0, 255)(bitgen));
                        });
    ...
    std::pair<std::string_view, const char*>>;
            ...
            using TurboContainerHashTypes = Types<TypeWithTurboContainerHash, TypeWithTurboContainerHashAndEq>;
            ...
            TYPED_TEST(TurboContainerHashTest, HeterogeneityInMapWorks) {
                fermat::flat_hash_map<TypeParam, int> map;
            ...
            TYPED_TEST(TurboContainerHashTest, HeterogeneityInSetWorks) {
                fermat::flat_hash_set<TypeParam> set;
