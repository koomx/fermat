...
namespace fermat {
    ...
    namespace container_internal {
        ...
        inline std::mt19937& HashTestRng() {
        ...
        struct Generator<T, std::enable_if_t<std::is_integral_v<T>>> {
            T operator()() const { return dist(HashTestRng()); }
            mutable std::uniform_int_distribution<T> dist;
        };

        template <>
        struct Generator<Enum> {
            Enum operator()() const { return static_cast<Enum>(dist(HashTestRng())); }
        ...
        struct Generator<EnumClass> {
            EnumClass operator()() const {
                return static_cast<EnumClass>(dist(HashTestRng()));
            }
