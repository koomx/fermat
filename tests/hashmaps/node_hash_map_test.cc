...
#include <tests/hashmaps/internal/unordered_map_constructor_test.h>
#include <tests/hashmaps/internal/unordered_map_lookup_test.h>
#include <tests/hashmaps/internal/unordered_map_members_test.h>
#include <tests/hashmaps/internal/unordered_map_modifiers_test.h>
...
using MapTypes = ::testing::Types<
    fermat::node_hash_map<int, int, StatefulTestingHash, StatefulTestingEqual,
                        Alloc<std::pair<const int, int>>>,
    fermat::node_hash_map<std::string, std::string, StatefulTestingHash,
...
using M = fermat::node_hash_map<std::string, fermat::base_internal::Tracked<int>>;

TEST(NodeHashMap, Emplace) {
  M m;
  fermat::base_internal::Tracked<int> t(53);
  m.emplace("a", t);
  ...
  std::pair<std::string, fermat::base_internal::Tracked<int>> p("a", t);
  ASSERT_EQ(0, t.num_moves());
  ...
  const std::pair<std::string, fermat::base_internal::Tracked<int>> cp("a", t);
  ASSERT_EQ(0, t.num_moves());
  ...
  std::pair<const std::string, fermat::base_internal::Tracked<int>> pc("a", t);
  ASSERT_EQ(0, t.num_moves());
  ...
  const std::pair<const std::string, fermat::base_internal::Tracked<int>> cpc("a", t);
  ASSERT_EQ(0, t.num_moves());
...
TEST(NodeHashMap, AssignRecursive) {
  struct Tree {
    ...
    fermat::node_hash_map<int, Tree> children;
  };
  Tree root;
...
TEST(FlatHashMap, MoveOnlyKey) {
  ...
  fermat::node_hash_map<Key, int, Hash, Eq> m;
  m[Key()];
}
...
TEST(NodeHashMap, MergeExtractInsert) {
  fermat::node_hash_map<NonMovableKey, int, NonMovableKeyHash, NonMovableKeyEq>
      set1, set2;
  ...
  for (int i = 0; i < 100; ++i) {
    {
      ...
      fermat::container_internal::c_for_each_fast(
...
TEST(NodeHashMap, FromRangeWithHasherAndAllocator) {
  ...
  fermat::node_hash_map<int, int, TestingHash,
                      fermat::container_internal::hash_default_eq<int>,
                      Alloc<std::pair<const int, int>>>
