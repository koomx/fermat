...
namespace {
    ...
    void BenchmarkTryEmplaceStrings(benchmark::State& state,
        ...
        const size_t batch_size = std::max(size_t { 1 }, size_t { 1000000 } / data.str_bytes);
        std::vector<fermat::linked_hash_map<std::string, int>> sets(batch_size);
    ...
    constexpr std::string_view kFormatLong = "a longer string that exceeds the SSO %10d";
