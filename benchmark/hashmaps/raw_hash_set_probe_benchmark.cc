...
namespace {
    ...
    enum class OutputStyle { kRegular,
        kBenchmark };
    ...
    public:
        ...
        static T* allocate(size_t n) {
            ...
            // Choose a random one.
            size_t i = std::uniform_int_distribution<size_t>(0, pointers.size() - 1)(
                GlobalBitGen());
            T* result = pointers[i];
    ...
    private:
        ...
        static std::vector<T*>& GetPointers(size_t n) {
            static turbo::NoDestructor<fermat::flat_hash_map<size_t, std::vector<T*>>> m;
            return (*m)[n];
        }
    };
    ...
    template <class T>
    struct DefaultHash {
        using type = fermat::container_internal::hash_default_hash<T>;
    };
    ...
    template <class T>
    struct Table : fermat::container_internal::raw_hash_set<
                       Policy<T>, DefaultHashT<T>,
                       fermat::container_internal::hash_default_eq<T>,
    ...
    Ratios CollectMeanProbeLengths() {
        ...
        while (t.size() < min_max_sizes.min_load)
            t.insert(elem());
        result.min_load = fermat::container_internal::GetHashtableDebugProbeSummary(t).mean;

        while (t.size() < (min_max_sizes.min_load + min_max_sizes.max_load) / 2)
            t.insert(elem());
        result.avg_load = fermat::container_internal::GetHashtableDebugProbeSummary(t).mean;

        while (t.size() < min_max_sizes.max_load)
            t.insert(elem());
        result.max_load = fermat::container_internal::GetHashtableDebugProbeSummary(t).mean;
    ...
    struct AlmostSequential {
        ...
        auto operator()() const -> decltype(current()) {
            while (std::uniform_real_distribution<double>(0.0, 1.0)(GlobalBitGen()) <=
                   percent_skip / 100.)
                current();
            return current();
        }
    ...
    struct Uniform {
        template <typename T>
        T operator()(T) const {
            return std::uniform_int_distribution<T>(T { 0 }, ~T { 0 })(GlobalBitGen());
        }
    };
    ...
    struct Gaussian {
        template <typename T>
        T operator()(T) const {
            std::normal_distribution<double> dist(1e6, 1e4);
            double d;
            do {
                d = dist(GlobalBitGen());
    ...
    struct Zipf {
        template <typename T>
        T operator()(T) const {
            // Inverse-transform sample of Zipf with q = 1.6 over [1, max].
            ...
            for (;;) {
                double k = std::pow(u(GlobalBitGen()), exp);
                if (k >= 1.0 && k <= maxv)
    ...
    void RunForTypeAndDistribution(std::vector<Result>& results) {
        ...
        if (!CanRunBenchmark(turbo::str_cat(name, "/min")) && !CanRunBenchmark(turbo::str_cat(name, "/avg")) && !CanRunBenchmark(turbo::str_cat(name, "/max"))) {
            return;
        }
    ...
    case OutputStyle::kBenchmark: {
        ...
        for (const auto& result : results) {
            auto print = [&](std::string_view stat, double Ratios::* val) {
                std::string name = turbo::str_cat(result.name, "/", result.dist_name, "/", stat);
