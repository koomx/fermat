            ...
            void BM_EraseIf(benchmark::State& state) {
                ...
                for (auto& k : keys) {
                    ...
                    for (int64_t i = 0; i < num_elements; i++) {
                        ...
                        k.push_back(std::uniform_int_distribution<int64_t>(
                            0, std::numeric_limits<int64_t>::max() - 1)(rng));
...
// parts of the API.
auto CodegenTurboRawHashSetInt64Find(fermat::container_internal::IntTable* table,
    int64_t key) -> decltype(table->find(key)) {
    return table->find(key);
}

bool CodegenTurboRawHashSetInt64FindNeEnd(
    fermat::container_internal::IntTable* table, int64_t key) {
...
bool CodegenTurboRawHashSetStringFindNeEnd(
    fermat::container_internal::StringTable* table, const std::string& key) {
    return table->find(key) != table->end();
}

auto CodegenTurboRawHashSetInt64Insert(fermat::container_internal::IntTable* table,
    int64_t key)
...
void CodegenTurboRawHashSetInt64Iterate(
    fermat::container_internal::IntTable* table) {
    for (auto x : *table)
