...
#define BTREE_TYPES(value)                                                                             \
    ...
    using btree_256_map_##value = btree_map<value, intptr_t, std::less<value>,                         \
    ...
    using btree_256_multimap_##value = btree_multimap<value, intptr_t, std::less<value>,               \
        std::allocator<std::pair<const value, intptr_t>>>
...
#define BIG_TYPE_BENCHMARKS(SIZE, COPIES)                                                                              \
    ...
    using stl_unordered_set_size##SIZE##copies##COPIES = std::unordered_set<BigType<SIZE, COPIES>,                     \
        turbo::Hash<BigType<SIZE, COPIES>>>;                                                                           \
    using stl_unordered_map_size##SIZE##copies##COPIES = std::unordered_map<BigType<SIZE, COPIES>, intptr_t,           \
        turbo::Hash<BigType<SIZE, COPIES>>>;                                                                           \
    ...
    using stl_unordered_multiset_size##SIZE##copies##COPIES = std::unordered_multiset<BigType<SIZE, COPIES>,           \
        turbo::Hash<BigType<SIZE, COPIES>>>;                                                                           \
    using stl_unordered_multimap_size##SIZE##copies##COPIES = std::unordered_multimap<BigType<SIZE, COPIES>, intptr_t, \
        turbo::Hash<BigType<SIZE, COPIES>>>;                                                                           \
...
#define BIG_TYPE_PTR_BENCHMARKS(SIZE)                                                                     \
    ...
    using stl_unordered_set_size##SIZE##copies##SIZE##ptr = std::unordered_set<BigType<SIZE, SIZE>,       \
    ...
    MY_BENCHMARK3(btree_256_map_size##SIZE##copies##SIZE##ptr)
            ...
            void BM_BtreeSet_IteratorDifference(benchmark::State& state) {
                ...
                size_t distance = std::uniform_int_distribution<size_t>(0, set.size() - 1)(bitgen);
                while (state.KeepRunningBatch(distance)) {
                    size_t end = std::uniform_int_distribution<size_t>(distance, set.size() - 1)(bitgen);
                    size_t begin = end - distance;
                    benchmark::DoNotOptimize(set.find(static_cast<int>(end)) - set.find(static_cast<int>(begin)));
                    distance = std::uniform_int_distribution<size_t>(0, set.size() - 1)(bitgen);
                }
            }
